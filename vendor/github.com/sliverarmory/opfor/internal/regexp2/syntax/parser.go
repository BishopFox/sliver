...
const (
	...
	ErrMutuallyExclusiveCaseModes = "case-insensitive modes are mutually exclusive"
)
...
func Parse(re string, op RegexOptions) (*RegexTree, error) {
	if op&customCaseOptions == customCaseOptions ||
		(op&IgnoreCase != 0 && op&customCaseOptions != 0) {
		return nil, &Error{Code: ErrMutuallyExclusiveCaseModes, Expr: re}
	}
...
func (p *parser) scanBackslash(scanOnly bool) (*regexNode, error) {
	...
	case 'b', 'B', 'A', 'G', 'Z', 'z':
		if ch == 'b' && p.charsRight() >= 4 &&
			p.rightChar(1) == '{' && p.rightChar(2) == 'g' && p.rightChar(3) == '}' {
			p.moveRight(4)
			return newRegexNode(ntGraphemeBoundary, p.options), nil
		}
		p.moveRight(1)
		...
		if ch == '-' {
			...
			if off {
				...
				if option == IgnoreCase {
					p.options &= ^customCaseOptions
				} else if option == JavaASCII {
					p.options &= ^(IgnoreCase | JavaUnicode)
				} else if option == JavaUnicode {
					p.options &= ^(IgnoreCase | JavaASCII)
				}
				p.options |= option
			}
		}
...
func (p *parser) caseMode() CaseMode {
	return caseModeFromOptions(p.options)
}
...
func (p *parser) addToConcatenate(pos, cch int, isReplacement bool) {
	...
	if cch > 1 {
		...
		if p.caseMode() != CaseSensitive && !isReplacement {
			// We do the ToLower character by character for consistency.  With surrogate chars, doing
			// a ToLower on the entire string could actually change the surrogate pair.  This is more correct
			// linguistically, but since Regex doesn't support surrogates, it's more important to be
			// consistent.
			for i := 0; i < len(str); i++ {
				str[i] = FoldCase(str[i], p.caseMode())
			}
		}
