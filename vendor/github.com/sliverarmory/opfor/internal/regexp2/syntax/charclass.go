...
func (c CharSet) CharIn(ch rune) bool {
	return c.charInVariants([]rune{ch})
}

// CharInCase tests membership after applying a Java-compatible case closure.
// The closure is evaluated against the original ranges so special equivalents
// such as Kelvin sign, long s, dotless i, and final sigma work in ranges too.
func (c CharSet) CharInCase(ch rune, mode CaseMode) bool {
	switch mode {
	case CaseJavaASCII:
		key := FoldCase(ch, mode)
		if 'a' <= key && key <= 'z' {
			return c.charInVariants([]rune{key, key - ('a' - 'A')})
		}
		return c.charInVariants([]rune{key})
	case CaseJavaUnicode:
		if variants := javaUnicodeVariants(ch); len(variants) != 0 {
			return c.charInVariants(variants)
		}
		return c.charInVariants([]rune{ch})
	default:
		return c.CharIn(ch)
...
func (c CharSet) charInVariants(variants []rune) bool {
	val := false
	for _, ch := range variants {
		if c.charInPositive(ch) {
			val = true
			break
		}
	}
	...
	if val && c.sub != nil {
		val = !c.sub.charInVariants(variants)
	}
	return val
}

func (c CharSet) charInPositive(ch rune) bool {
	for _, r := range c.ranges {
	...
	for _, ct := range c.categories {
		...
		case spaceCategoryText:
			matched = unicode.IsSpace(ch)
		case wordCategoryText:
			matched = IsWordChar(ch)
		default:
			matched = unicode.Is(unicodeCategories[ct.cat], ch)
		}
		if matched || ct.negate {
			return matched != ct.negate
		}
	}
	return false
}
