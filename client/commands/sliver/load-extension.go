...
func (l *LoadExtension) Execute(args []string) (err error) {
	...
	// We will need the current command parser, to add commands to it
	...
	var bindExtensionCommands = func() {
		...
		root, err := parser.AddCommand(ext.Name,
			fmt.Sprintf("%s extension commands", ext.Name), "",
			&ExtensionCommand{})
		cctx.Commands.RegisterSliverCommand(err, root, constants.ExtensionsGroup)
		...
		for _, extCmd := range ext.Commands {
			...
			// Add the subcommand
			sub, err := root.AddCommand(extCmd.Name,
				extCmd.Help,
				help.FormatHelpTmpl(extCmd.LongHelp),
				&ExtensionCommand{root: ext, sub: &extCmd},
			)
			cctx.Commands.RegisterSliverCommand(err, nil, constants.ExtensionsGroup)
			...
			if extCmd.IsAssembly {
				...
				sub.FindOptionByLongName("arch").Choices = extensionArch
			}
		}
	}

	// Bind the extension to a given session. Create the extensions map if needed.
	sessionExtensions, found := LoadedExtensions[cctx.Context.Sliver.ID]
	if found {
		sessionExtensions[ext.Path] = bindExtensionCommands
	} else {
		LoadedExtensions[cctx.Context.Sliver.ID] = map[string]func(){}
		LoadedExtensions[cctx.Context.Sliver.ID][ext.Path] = bindExtensionCommands
	}
