// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v3.15.8
// source: rpcpb/services.proto

package rpcpb

import (
	clientpb "github.com/bishopfox/sliver/protobuf/clientpb"
	commonpb "github.com/bishopfox/sliver/protobuf/commonpb"
	sliverpb "github.com/bishopfox/sliver/protobuf/sliverpb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_rpcpb_services_proto protoreflect.FileDescriptor

var file_rpcpb_services_proto_rawDesc = []byte{
	,
}

var file_rpcpb_services_proto_goTypes = []interface{}{
	(*commonpb.Empty)(nil),                    // 0: commonpb.Empty
	(*clientpb.ClientLogData)(nil),            // 1: clientpb.ClientLogData
	(*sliverpb.KillReq)(nil),                  // 2: sliverpb.KillReq
	(*sliverpb.ReconfigureReq)(nil),           // 3: sliverpb.ReconfigureReq
	(*clientpb.RenameReq)(nil),                // 4: clientpb.RenameReq
	(*clientpb.MonitoringProvider)(nil),       // 5: clientpb.MonitoringProvider
	(*clientpb.MTLSListenerReq)(nil),          // 6: clientpb.MTLSListenerReq
	(*clientpb.WGListenerReq)(nil),            // 7: clientpb.WGListenerReq
	(*clientpb.DNSListenerReq)(nil),           // 8: clientpb.DNSListenerReq
	(*clientpb.HTTPListenerReq)(nil),          // 9: clientpb.HTTPListenerReq
	(*clientpb.Beacon)(nil),                   // 10: clientpb.Beacon
	(*clientpb.BeaconTask)(nil),               // 11: clientpb.BeaconTask
	(*clientpb.BeaconIntegrity)(nil),          // 12: clientpb.BeaconIntegrity
	(*clientpb.KillJobReq)(nil),               // 13: clientpb.KillJobReq
	(*clientpb.RestartJobReq)(nil),            // 14: clientpb.RestartJobReq
	(*clientpb.StagerListenerReq)(nil),        // 15: clientpb.StagerListenerReq
	(*clientpb.Loot)(nil),                     // 16: clientpb.Loot
	(*clientpb.Credentials)(nil),              // 17: clientpb.Credentials
	(*clientpb.Credential)(nil),               // 18: clientpb.Credential
	(*clientpb.Host)(nil),                     // 19: clientpb.Host
	(*clientpb.IOC)(nil),                      // 20: clientpb.IOC
	(*clientpb.GenerateReq)(nil),              // 21: clientpb.GenerateReq
	(*clientpb.ExternalGenerateReq)(nil),      // 22: clientpb.ExternalGenerateReq
	(*clientpb.ExternalImplantBinary)(nil),    // 23: clientpb.ExternalImplantBinary
	(*clientpb.ImplantBuild)(nil),             // 24: clientpb.ImplantBuild
	(*clientpb.GenerateStageReq)(nil),         // 25: clientpb.GenerateStageReq
	(*clientpb.ImplantStageReq)(nil),          // 26: clientpb.ImplantStageReq
	(*clientpb.C2ProfileReq)(nil),             // 27: clientpb.C2ProfileReq
	(*clientpb.HTTPC2ConfigReq)(nil),          // 28: clientpb.HTTPC2ConfigReq
	(*clientpb.Builder)(nil),                  // 29: clientpb.Builder
	(*clientpb.Event)(nil),                    // 30: clientpb.Event
	(*clientpb.CertificatesReq)(nil),          // 31: clientpb.CertificatesReq
	(*clientpb.Crackstation)(nil),             // 32: clientpb.Crackstation
	(*clientpb.CrackBenchmark)(nil),           // 33: clientpb.CrackBenchmark
	(*clientpb.CrackTask)(nil),                // 34: clientpb.CrackTask
	(*clientpb.CrackFile)(nil),                // 35: clientpb.CrackFile
	(*clientpb.CrackFileChunk)(nil),           // 36: clientpb.CrackFileChunk
	(*clientpb.RegenerateReq)(nil),            // 37: clientpb.RegenerateReq
	(*clientpb.DeleteReq)(nil),                // 38: clientpb.DeleteReq
	(*clientpb.ImplantProfile)(nil),           // 39: clientpb.ImplantProfile
	(*clientpb.ShellcodeRDIReq)(nil),          // 40: clientpb.ShellcodeRDIReq
	(*clientpb.ShellcodeEncodeReq)(nil),       // 41: clientpb.ShellcodeEncodeReq
	(*clientpb.TrafficEncoder)(nil),           // 42: clientpb.TrafficEncoder
	(*clientpb.Website)(nil),                  // 43: clientpb.Website
	(*clientpb.WebsiteAddContent)(nil),        // 44: clientpb.WebsiteAddContent
	(*clientpb.WebsiteRemoveContent)(nil),     // 45: clientpb.WebsiteRemoveContent
	(*sliverpb.Ping)(nil),                     // 46: sliverpb.Ping
	(*sliverpb.PsReq)(nil),                    // 47: sliverpb.PsReq
	(*sliverpb.TerminateReq)(nil),             // 48: sliverpb.TerminateReq
	(*sliverpb.IfconfigReq)(nil),              // 49: sliverpb.IfconfigReq
	(*sliverpb.NetstatReq)(nil),               // 50: sliverpb.NetstatReq
	(*sliverpb.LsReq)(nil),                    // 51: sliverpb.LsReq
	(*sliverpb.CdReq)(nil),                    // 52: sliverpb.CdReq
	(*sliverpb.PwdReq)(nil),                   // 53: sliverpb.PwdReq
	(*sliverpb.MvReq)(nil),                    // 54: sliverpb.MvReq
	(*sliverpb.CpReq)(nil),                    // 55: sliverpb.CpReq
	(*sliverpb.RmReq)(nil),                    // 56: sliverpb.RmReq
	(*sliverpb.MkdirReq)(nil),                 // 57: sliverpb.MkdirReq
	(*sliverpb.DownloadReq)(nil),              // 58: sliverpb.DownloadReq
	(*sliverpb.UploadReq)(nil),                // 59: sliverpb.UploadReq
	(*sliverpb.GrepReq)(nil),                  // 60: sliverpb.GrepReq
	(*sliverpb.ChmodReq)(nil),                 // 61: sliverpb.ChmodReq
	(*sliverpb.ChownReq)(nil),                 // 62: sliverpb.ChownReq
	(*sliverpb.ChtimesReq)(nil),               // 63: sliverpb.ChtimesReq
	(*sliverpb.MemfilesListReq)(nil),          // 64: sliverpb.MemfilesListReq
	(*sliverpb.MemfilesAddReq)(nil),           // 65: sliverpb.MemfilesAddReq
	(*sliverpb.MemfilesRmReq)(nil),            // 66: sliverpb.MemfilesRmReq
	(*sliverpb.MountReq)(nil),                 // 67: sliverpb.MountReq
	(*sliverpb.ProcessDumpReq)(nil),           // 68: sliverpb.ProcessDumpReq
	(*sliverpb.RunAsReq)(nil),                 // 69: sliverpb.RunAsReq
	(*sliverpb.ImpersonateReq)(nil),           // 70: sliverpb.ImpersonateReq
	(*sliverpb.RevToSelfReq)(nil),             // 71: sliverpb.RevToSelfReq
	(*clientpb.GetSystemReq)(nil),             // 72: clientpb.GetSystemReq
	(*sliverpb.TaskReq)(nil),                  // 73: sliverpb.TaskReq
	(*clientpb.MSFReq)(nil),                   // 74: clientpb.MSFReq
	(*clientpb.MSFRemoteReq)(nil),             // 75: clientpb.MSFRemoteReq
	(*sliverpb.ExecuteAssemblyReq)(nil),       // 76: sliverpb.ExecuteAssemblyReq
	(*clientpb.MigrateReq)(nil),               // 77: clientpb.MigrateReq
	(*sliverpb.ExecuteReq)(nil),               // 78: sliverpb.ExecuteReq
	(*sliverpb.ExecuteWindowsReq)(nil),        // 79: sliverpb.ExecuteWindowsReq
	(*sliverpb.ExecuteChildrenReq)(nil),       // 80: sliverpb.ExecuteChildrenReq
	(*sliverpb.SideloadReq)(nil),              // 81: sliverpb.SideloadReq
	(*sliverpb.InvokeSpawnDllReq)(nil),        // 82: sliverpb.InvokeSpawnDllReq
	(*sliverpb.ScreenshotReq)(nil),            // 83: sliverpb.ScreenshotReq
	(*sliverpb.CurrentTokenOwnerReq)(nil),     // 84: sliverpb.CurrentTokenOwnerReq
	(*sliverpb.ServicesReq)(nil),              // 85: sliverpb.ServicesReq
	(*sliverpb.ServiceDetailReq)(nil),         // 86: sliverpb.ServiceDetailReq
	(*sliverpb.StartServiceByNameReq)(nil),    // 87: sliverpb.StartServiceByNameReq
	(*sliverpb.PivotStartListenerReq)(nil),    // 88: sliverpb.PivotStartListenerReq
	(*sliverpb.PivotStopListenerReq)(nil),     // 89: sliverpb.PivotStopListenerReq
	(*sliverpb.PivotListenersReq)(nil),        // 90: sliverpb.PivotListenersReq
	(*sliverpb.StartServiceReq)(nil),          // 91: sliverpb.StartServiceReq
	(*sliverpb.StopServiceReq)(nil),           // 92: sliverpb.StopServiceReq
	(*sliverpb.RemoveServiceReq)(nil),         // 93: sliverpb.RemoveServiceReq
	(*sliverpb.MakeTokenReq)(nil),             // 94: sliverpb.MakeTokenReq
	(*sliverpb.EnvReq)(nil),                   // 95: sliverpb.EnvReq
	(*sliverpb.SetEnvReq)(nil),                // 96: sliverpb.SetEnvReq
	(*sliverpb.UnsetEnvReq)(nil),              // 97: sliverpb.UnsetEnvReq
	(*clientpb.BackdoorReq)(nil),              // 98: clientpb.BackdoorReq
	(*sliverpb.RegistryReadReq)(nil),          // 99: sliverpb.RegistryReadReq
	(*sliverpb.RegistryWriteReq)(nil),         // 100: sliverpb.RegistryWriteReq
	(*sliverpb.RegistryCreateKeyReq)(nil),     // 101: sliverpb.RegistryCreateKeyReq
	(*sliverpb.RegistryDeleteKeyReq)(nil),     // 102: sliverpb.RegistryDeleteKeyReq
	(*sliverpb.RegistrySubKeyListReq)(nil),    // 103: sliverpb.RegistrySubKeyListReq
	(*sliverpb.RegistryListValuesReq)(nil),    // 104: sliverpb.RegistryListValuesReq
	(*sliverpb.RegistryReadHiveReq)(nil),      // 105: sliverpb.RegistryReadHiveReq
	(*sliverpb.SSHCommandReq)(nil),            // 106: sliverpb.SSHCommandReq
	(*clientpb.DllHijackReq)(nil),             // 107: clientpb.DllHijackReq
	(*sliverpb.GetPrivsReq)(nil),              // 108: sliverpb.GetPrivsReq
	(*sliverpb.RportFwdStartListenerReq)(nil), // 109: sliverpb.RportFwdStartListenerReq
	(*sliverpb.RportFwdListenersReq)(nil),     // 110: sliverpb.RportFwdListenersReq
	(*sliverpb.RportFwdStopListenerReq)(nil),  // 111: sliverpb.RportFwdStopListenerReq
	(*sliverpb.OpenSession)(nil),              // 112: sliverpb.OpenSession
	(*sliverpb.CloseSession)(nil),             // 113: sliverpb.CloseSession
	(*sliverpb.RegisterExtensionReq)(nil),     // 114: sliverpb.RegisterExtensionReq
	(*sliverpb.CallExtensionReq)(nil),         // 115: sliverpb.CallExtensionReq
	(*sliverpb.ListExtensionsReq)(nil),        // 116: sliverpb.ListExtensionsReq
	(*sliverpb.RegisterWasmExtensionReq)(nil), // 117: sliverpb.RegisterWasmExtensionReq
	(*sliverpb.ListWasmExtensionsReq)(nil),    // 118: sliverpb.ListWasmExtensionsReq
	(*sliverpb.ExecWasmExtensionReq)(nil),     // 119: sliverpb.ExecWasmExtensionReq
	(*sliverpb.WGPortForwardStartReq)(nil),    // 120: sliverpb.WGPortForwardStartReq
	(*sliverpb.WGPortForwardStopReq)(nil),     // 121: sliverpb.WGPortForwardStopReq
	(*sliverpb.WGSocksStartReq)(nil),          // 122: sliverpb.WGSocksStartReq
	(*sliverpb.WGSocksStopReq)(nil),           // 123: sliverpb.WGSocksStopReq
	(*sliverpb.WGTCPForwardersReq)(nil),       // 124: sliverpb.WGTCPForwardersReq
	(*sliverpb.WGSocksServersReq)(nil),        // 125: sliverpb.WGSocksServersReq
	(*sliverpb.ShellReq)(nil),                 // 126: sliverpb.ShellReq
	(*sliverpb.ShellResizeReq)(nil),           // 127: sliverpb.ShellResizeReq
	(*sliverpb.PortfwdReq)(nil),               // 128: sliverpb.PortfwdReq
	(*sliverpb.Socks)(nil),                    // 129: sliverpb.Socks
	(*sliverpb.SocksData)(nil),                // 130: sliverpb.SocksData
	(*sliverpb.Tunnel)(nil),                   // 131: sliverpb.Tunnel
	(*sliverpb.TunnelData)(nil),               // 132: sliverpb.TunnelData
	(*clientpb.Version)(nil),                  // 133: clientpb.Version
	(*clientpb.Operators)(nil),                // 134: clientpb.Operators
	(*sliverpb.Reconfigure)(nil),              // 135: sliverpb.Reconfigure
	(*clientpb.Sessions)(nil),                 // 136: clientpb.Sessions
	(*commonpb.Response)(nil),                 // 137: commonpb.Response
	(*clientpb.MonitoringProviders)(nil),      // 138: clientpb.MonitoringProviders
	(*clientpb.ListenerJob)(nil),              // 139: clientpb.ListenerJob
	(*clientpb.Beacons)(nil),                  // 140: clientpb.Beacons
	(*clientpb.BeaconTasks)(nil),              // 141: clientpb.BeaconTasks
	(*clientpb.Jobs)(nil),                     // 142: clientpb.Jobs
	(*clientpb.KillJob)(nil),                  // 143: clientpb.KillJob
	(*clientpb.StagerListener)(nil),           // 144: clientpb.StagerListener
	(*clientpb.AllLoot)(nil),                  // 145: clientpb.AllLoot
	(*clientpb.AllHosts)(nil),                 // 146: clientpb.AllHosts
	(*clientpb.Generate)(nil),                 // 147: clientpb.Generate
	(*clientpb.ExternalImplantConfig)(nil),    // 148: clientpb.ExternalImplantConfig
	(*clientpb.HTTPC2Configs)(nil),            // 149: clientpb.HTTPC2Configs
	(*clientpb.HTTPC2Config)(nil),             // 150: clientpb.HTTPC2Config
	(*clientpb.Builders)(nil),                 // 151: clientpb.Builders
	(*clientpb.CertificateInfo)(nil),          // 152: clientpb.CertificateInfo
	(*clientpb.CertificateAuthorityInfo)(nil), // 153: clientpb.CertificateAuthorityInfo
	(*clientpb.Crackstations)(nil),            // 154: clientpb.Crackstations
	(*clientpb.CrackFiles)(nil),               // 155: clientpb.CrackFiles
	(*clientpb.ImplantBuilds)(nil),            // 156: clientpb.ImplantBuilds
	(*clientpb.Canaries)(nil),                 // 157: clientpb.Canaries
	(*clientpb.WGClientConfig)(nil),           // 158: clientpb.WGClientConfig
	(*clientpb.UniqueWGIP)(nil),               // 159: clientpb.UniqueWGIP
	(*clientpb.ImplantProfiles)(nil),          // 160: clientpb.ImplantProfiles
	(*clientpb.ShellcodeRDI)(nil),             // 161: clientpb.ShellcodeRDI
	(*clientpb.Compiler)(nil),                 // 162: clientpb.Compiler
	(*clientpb.ShellcodeEncode)(nil),          // 163: clientpb.ShellcodeEncode
	(*clientpb.ShellcodeEncoderMap)(nil),      // 164: clientpb.ShellcodeEncoderMap
	(*clientpb.TrafficEncoderMap)(nil),        // 165: clientpb.TrafficEncoderMap
	(*clientpb.TrafficEncoderTests)(nil),      // 166: clientpb.TrafficEncoderTests
	(*clientpb.Websites)(nil),                 // 167: clientpb.Websites
	(*sliverpb.Ps)(nil),                       // 168: sliverpb.Ps
	(*sliverpb.Terminate)(nil),                // 169: sliverpb.Terminate
	(*sliverpb.Ifconfig)(nil),                 // 170: sliverpb.Ifconfig
	(*sliverpb.Netstat)(nil),                  // 171: sliverpb.Netstat
	(*sliverpb.Ls)(nil),                       // 172: sliverpb.Ls
	(*sliverpb.Pwd)(nil),                      // 173: sliverpb.Pwd
	(*sliverpb.Mv)(nil),                       // 174: sliverpb.Mv
	(*sliverpb.Cp)(nil),                       // 175: sliverpb.Cp
	(*sliverpb.Rm)(nil),                       // 176: sliverpb.Rm
	(*sliverpb.Mkdir)(nil),                    // 177: sliverpb.Mkdir
	(*sliverpb.Download)(nil),                 // 178: sliverpb.Download
	(*sliverpb.Upload)(nil),                   // 179: sliverpb.Upload
	(*sliverpb.Grep)(nil),                     // 180: sliverpb.Grep
	(*sliverpb.Chmod)(nil),                    // 181: sliverpb.Chmod
	(*sliverpb.Chown)(nil),                    // 182: sliverpb.Chown
	(*sliverpb.Chtimes)(nil),                  // 183: sliverpb.Chtimes
	(*sliverpb.MemfilesAdd)(nil),              // 184: sliverpb.MemfilesAdd
	(*sliverpb.MemfilesRm)(nil),               // 185: sliverpb.MemfilesRm
	(*sliverpb.Mount)(nil),                    // 186: sliverpb.Mount
	(*sliverpb.ProcessDump)(nil),              // 187: sliverpb.ProcessDump
	(*sliverpb.RunAs)(nil),                    // 188: sliverpb.RunAs
	(*sliverpb.Impersonate)(nil),              // 189: sliverpb.Impersonate
	(*sliverpb.RevToSelf)(nil),                // 190: sliverpb.RevToSelf
	(*sliverpb.GetSystem)(nil),                // 191: sliverpb.GetSystem
	(*sliverpb.Task)(nil),                     // 192: sliverpb.Task
	(*sliverpb.ExecuteAssembly)(nil),          // 193: sliverpb.ExecuteAssembly
	(*sliverpb.Migrate)(nil),                  // 194: sliverpb.Migrate
	(*sliverpb.Execute)(nil),                  // 195: sliverpb.Execute
	(*sliverpb.ExecuteChildren)(nil),          // 196: sliverpb.ExecuteChildren
	(*sliverpb.Sideload)(nil),                 // 197: sliverpb.Sideload
	(*sliverpb.SpawnDll)(nil),                 // 198: sliverpb.SpawnDll
	(*sliverpb.Screenshot)(nil),               // 199: sliverpb.Screenshot
	(*sliverpb.CurrentTokenOwner)(nil),        // 200: sliverpb.CurrentTokenOwner
	(*sliverpb.Services)(nil),                 // 201: sliverpb.Services
	(*sliverpb.ServiceDetail)(nil),            // 202: sliverpb.ServiceDetail
	(*sliverpb.ServiceInfo)(nil),              // 203: sliverpb.ServiceInfo
	(*sliverpb.PivotListener)(nil),            // 204: sliverpb.PivotListener
	(*sliverpb.PivotListeners)(nil),           // 205: sliverpb.PivotListeners
	(*clientpb.PivotGraph)(nil),               // 206: clientpb.PivotGraph
	(*sliverpb.MakeToken)(nil),                // 207: sliverpb.MakeToken
	(*sliverpb.EnvInfo)(nil),                  // 208: sliverpb.EnvInfo
	(*sliverpb.SetEnv)(nil),                   // 209: sliverpb.SetEnv
	(*sliverpb.UnsetEnv)(nil),                 // 210: sliverpb.UnsetEnv
	(*clientpb.Backdoor)(nil),                 // 211: clientpb.Backdoor
	(*sliverpb.RegistryRead)(nil),             // 212: sliverpb.RegistryRead
	(*sliverpb.RegistryWrite)(nil),            // 213: sliverpb.RegistryWrite
	(*sliverpb.RegistryCreateKey)(nil),        // 214: sliverpb.RegistryCreateKey
	(*sliverpb.RegistryDeleteKey)(nil),        // 215: sliverpb.RegistryDeleteKey
	(*sliverpb.RegistrySubKeyList)(nil),       // 216: sliverpb.RegistrySubKeyList
	(*sliverpb.RegistryValuesList)(nil),       // 217: sliverpb.RegistryValuesList
	(*sliverpb.RegistryReadHive)(nil),         // 218: sliverpb.RegistryReadHive
	(*sliverpb.SSHCommand)(nil),               // 219: sliverpb.SSHCommand
	(*clientpb.DllHijack)(nil),                // 220: clientpb.DllHijack
	(*sliverpb.GetPrivs)(nil),                 // 221: sliverpb.GetPrivs
	(*sliverpb.RportFwdListener)(nil),         // 222: sliverpb.RportFwdListener
	(*sliverpb.RportFwdListeners)(nil),        // 223: sliverpb.RportFwdListeners
	(*sliverpb.RegisterExtension)(nil),        // 224: sliverpb.RegisterExtension
	(*sliverpb.CallExtension)(nil),            // 225: sliverpb.CallExtension
	(*sliverpb.ListExtensions)(nil),           // 226: sliverpb.ListExtensions
	(*sliverpb.RegisterWasmExtension)(nil),    // 227: sliverpb.RegisterWasmExtension
	(*sliverpb.ListWasmExtensions)(nil),       // 228: sliverpb.ListWasmExtensions
	(*sliverpb.ExecWasmExtension)(nil),        // 229: sliverpb.ExecWasmExtension
	(*sliverpb.WGPortForward)(nil),            // 230: sliverpb.WGPortForward
	(*sliverpb.WGSocks)(nil),                  // 231: sliverpb.WGSocks
	(*sliverpb.WGTCPForwarders)(nil),          // 232: sliverpb.WGTCPForwarders
	(*sliverpb.WGSocksServers)(nil),           // 233: sliverpb.WGSocksServers
	(*sliverpb.Shell)(nil),                    // 234: sliverpb.Shell
	(*sliverpb.Portfwd)(nil),                  // 235: sliverpb.Portfwd
}
var file_rpcpb_services_proto_depIdxs = []int32{
	0,   // 0: rpcpb.SliverRPC.GetVersion:input_type -> commonpb.Empty
	1,   // 1: rpcpb.SliverRPC.ClientLog:input_type -> clientpb.ClientLogData
	0,   // 2: rpcpb.SliverRPC.GetOperators:input_type -> commonpb.Empty
	2,   // 3: rpcpb.SliverRPC.Kill:input_type -> sliverpb.KillReq
	3,   // 4: rpcpb.SliverRPC.Reconfigure:input_type -> sliverpb.ReconfigureReq
	4,   // 5: rpcpb.SliverRPC.Rename:input_type -> clientpb.RenameReq
	0,   // 6: rpcpb.SliverRPC.GetSessions:input_type -> commonpb.Empty
	0,   // 7: rpcpb.SliverRPC.MonitorStart:input_type -> commonpb.Empty
	0,   // 8: rpcpb.SliverRPC.MonitorStop:input_type -> commonpb.Empty
	0,   // 9: rpcpb.SliverRPC.MonitorListConfig:input_type -> commonpb.Empty
	5,   // 10: rpcpb.SliverRPC.MonitorAddConfig:input_type -> clientpb.MonitoringProvider
	5,   // 11: rpcpb.SliverRPC.MonitorDelConfig:input_type -> clientpb.MonitoringProvider
	6,   // 12: rpcpb.SliverRPC.StartMTLSListener:input_type -> clientpb.MTLSListenerReq
	7,   // 13: rpcpb.SliverRPC.StartWGListener:input_type -> clientpb.WGListenerReq
	8,   // 14: rpcpb.SliverRPC.StartDNSListener:input_type -> clientpb.DNSListenerReq
	9,   // 15: rpcpb.SliverRPC.StartHTTPSListener:input_type -> clientpb.HTTPListenerReq
	9,   // 16: rpcpb.SliverRPC.StartHTTPListener:input_type -> clientpb.HTTPListenerReq
	0,   // 17: rpcpb.SliverRPC.GetBeacons:input_type -> commonpb.Empty
	10,  // 18: rpcpb.SliverRPC.GetBeacon:input_type -> clientpb.Beacon
	10,  // 19: rpcpb.SliverRPC.RmBeacon:input_type -> clientpb.Beacon
	10,  // 20: rpcpb.SliverRPC.GetBeaconTasks:input_type -> clientpb.Beacon
	11,  // 21: rpcpb.SliverRPC.GetBeaconTaskContent:input_type -> clientpb.BeaconTask
	11,  // 22: rpcpb.SliverRPC.CancelBeaconTask:input_type -> clientpb.BeaconTask
	12,  // 23: rpcpb.SliverRPC.UpdateBeaconIntegrityInformation:input_type -> clientpb.BeaconIntegrity
	0,   // 24: rpcpb.SliverRPC.GetJobs:input_type -> commonpb.Empty
	13,  // 25: rpcpb.SliverRPC.KillJob:input_type -> clientpb.KillJobReq
	14,  // 26: rpcpb.SliverRPC.RestartJobs:input_type -> clientpb.RestartJobReq
	15,  // 27: rpcpb.SliverRPC.StartTCPStagerListener:input_type -> clientpb.StagerListenerReq
	16,  // 28: rpcpb.SliverRPC.LootAdd:input_type -> clientpb.Loot
	16,  // 29: rpcpb.SliverRPC.LootRm:input_type -> clientpb.Loot
	16,  // 30: rpcpb.SliverRPC.LootUpdate:input_type -> clientpb.Loot
	16,  // 31: rpcpb.SliverRPC.LootContent:input_type -> clientpb.Loot
	0,   // 32: rpcpb.SliverRPC.LootAll:input_type -> commonpb.Empty
	0,   // 33: rpcpb.SliverRPC.Creds:input_type -> commonpb.Empty
	17,  // 34: rpcpb.SliverRPC.CredsAdd:input_type -> clientpb.Credentials
	17,  // 35: rpcpb.SliverRPC.CredsRm:input_type -> clientpb.Credentials
	17,  // 36: rpcpb.SliverRPC.CredsUpdate:input_type -> clientpb.Credentials
	18,  // 37: rpcpb.SliverRPC.GetCredByID:input_type -> clientpb.Credential
	18,  // 38: rpcpb.SliverRPC.GetCredsByHashType:input_type -> clientpb.Credential
	18,  // 39: rpcpb.SliverRPC.GetPlaintextCredsByHashType:input_type -> clientpb.Credential
	18,  // 40: rpcpb.SliverRPC.CredsSniffHashType:input_type -> clientpb.Credential
	0,   // 41: rpcpb.SliverRPC.Hosts:input_type -> commonpb.Empty
	19,  // 42: rpcpb.SliverRPC.Host:input_type -> clientpb.Host
	19,  // 43: rpcpb.SliverRPC.HostRm:input_type -> clientpb.Host
	20,  // 44: rpcpb.SliverRPC.HostIOCRm:input_type -> clientpb.IOC
	21,  // 45: rpcpb.SliverRPC.Generate:input_type -> clientpb.GenerateReq
	22,  // 46: rpcpb.SliverRPC.GenerateExternal:input_type -> clientpb.ExternalGenerateReq
	23,  // 47: rpcpb.SliverRPC.GenerateExternalSaveBuild:input_type -> clientpb.ExternalImplantBinary
	24,  // 48: rpcpb.SliverRPC.GenerateExternalGetBuildConfig:input_type -> clientpb.ImplantBuild
	25,  // 49: rpcpb.SliverRPC.GenerateStage:input_type -> clientpb.GenerateStageReq
	26,  // 50: rpcpb.SliverRPC.StageImplantBuild:input_type -> clientpb.ImplantStageReq
	0,   // 51: rpcpb.SliverRPC.GetHTTPC2Profiles:input_type -> commonpb.Empty
	27,  // 52: rpcpb.SliverRPC.GetHTTPC2ProfileByName:input_type -> clientpb.C2ProfileReq
	28,  // 53: rpcpb.SliverRPC.SaveHTTPC2Profile:input_type -> clientpb.HTTPC2ConfigReq
	29,  // 54: rpcpb.SliverRPC.BuilderRegister:input_type -> clientpb.Builder
	30,  // 55: rpcpb.SliverRPC.BuilderTrigger:input_type -> clientpb.Event
	0,   // 56: rpcpb.SliverRPC.Builders:input_type -> commonpb.Empty
	31,  // 57: rpcpb.SliverRPC.GetCertificateInfo:input_type -> clientpb.CertificatesReq
	0,   // 58: rpcpb.SliverRPC.GetCertificateAuthorityInfo:input_type -> commonpb.Empty
	32,  // 59: rpcpb.SliverRPC.CrackstationRegister:input_type -> clientpb.Crackstation
	30,  // 60: rpcpb.SliverRPC.CrackstationTrigger:input_type -> clientpb.Event
	33,  // 61: rpcpb.SliverRPC.CrackstationBenchmark:input_type -> clientpb.CrackBenchmark
	0,   // 62: rpcpb.SliverRPC.Crackstations:input_type -> commonpb.Empty
	34,  // 63: rpcpb.SliverRPC.CrackTaskByID:input_type -> clientpb.CrackTask
	34,  // 64: rpcpb.SliverRPC.CrackTaskUpdate:input_type -> clientpb.CrackTask
	35,  // 65: rpcpb.SliverRPC.CrackFilesList:input_type -> clientpb.CrackFile
	35,  // 66: rpcpb.SliverRPC.CrackFileCreate:input_type -> clientpb.CrackFile
	36,  // 67: rpcpb.SliverRPC.CrackFileChunkUpload:input_type -> clientpb.CrackFileChunk
	36,  // 68: rpcpb.SliverRPC.CrackFileChunkDownload:input_type -> clientpb.CrackFileChunk
	35,  // 69: rpcpb.SliverRPC.CrackFileComplete:input_type -> clientpb.CrackFile
	35,  // 70: rpcpb.SliverRPC.CrackFileDelete:input_type -> clientpb.CrackFile
	37,  // 71: rpcpb.SliverRPC.Regenerate:input_type -> clientpb.RegenerateReq
	0,   // 72: rpcpb.SliverRPC.ImplantBuilds:input_type -> commonpb.Empty
	38,  // 73: rpcpb.SliverRPC.DeleteImplantBuild:input_type -> clientpb.DeleteReq
	0,   // 74: rpcpb.SliverRPC.Canaries:input_type -> commonpb.Empty
	0,   // 75: rpcpb.SliverRPC.GenerateWGClientConfig:input_type -> commonpb.Empty
	0,   // 76: rpcpb.SliverRPC.GenerateUniqueIP:input_type -> commonpb.Empty
	0,   // 77: rpcpb.SliverRPC.ImplantProfiles:input_type -> commonpb.Empty
	38,  // 78: rpcpb.SliverRPC.DeleteImplantProfile:input_type -> clientpb.DeleteReq
	39,  // 79: rpcpb.SliverRPC.SaveImplantProfile:input_type -> clientpb.ImplantProfile
	40,  // 80: rpcpb.SliverRPC.ShellcodeRDI:input_type -> clientpb.ShellcodeRDIReq
	0,   // 81: rpcpb.SliverRPC.GetCompiler:input_type -> commonpb.Empty
	41,  // 82: rpcpb.SliverRPC.ShellcodeEncoder:input_type -> clientpb.ShellcodeEncodeReq
	0,   // 83: rpcpb.SliverRPC.ShellcodeEncoderMap:input_type -> commonpb.Empty
	0,   // 84: rpcpb.SliverRPC.TrafficEncoderMap:input_type -> commonpb.Empty
	42,  // 85: rpcpb.SliverRPC.TrafficEncoderAdd:input_type -> clientpb.TrafficEncoder
	42,  // 86: rpcpb.SliverRPC.TrafficEncoderRm:input_type -> clientpb.TrafficEncoder
	0,   // 87: rpcpb.SliverRPC.Websites:input_type -> commonpb.Empty
	43,  // 88: rpcpb.SliverRPC.Website:input_type -> clientpb.Website
	43,  // 89: rpcpb.SliverRPC.WebsiteRemove:input_type -> clientpb.Website
	44,  // 90: rpcpb.SliverRPC.WebsiteAddContent:input_type -> clientpb.WebsiteAddContent
	44,  // 91: rpcpb.SliverRPC.WebsiteUpdateContent:input_type -> clientpb.WebsiteAddContent
	45,  // 92: rpcpb.SliverRPC.WebsiteRemoveContent:input_type -> clientpb.WebsiteRemoveContent
	46,  // 93: rpcpb.SliverRPC.Ping:input_type -> sliverpb.Ping
	47,  // 94: rpcpb.SliverRPC.Ps:input_type -> sliverpb.PsReq
	48,  // 95: rpcpb.SliverRPC.Terminate:input_type -> sliverpb.TerminateReq
	49,  // 96: rpcpb.SliverRPC.Ifconfig:input_type -> sliverpb.IfconfigReq
	50,  // 97: rpcpb.SliverRPC.Netstat:input_type -> sliverpb.NetstatReq
	51,  // 98: rpcpb.SliverRPC.Ls:input_type -> sliverpb.LsReq
	52,  // 99: rpcpb.SliverRPC.Cd:input_type -> sliverpb.CdReq
	53,  // 100: rpcpb.SliverRPC.Pwd:input_type -> sliverpb.PwdReq
	54,  // 101: rpcpb.SliverRPC.Mv:input_type -> sliverpb.MvReq
	55,  // 102: rpcpb.SliverRPC.Cp:input_type -> sliverpb.CpReq
	56,  // 103: rpcpb.SliverRPC.Rm:input_type -> sliverpb.RmReq
	57,  // 104: rpcpb.SliverRPC.Mkdir:input_type -> sliverpb.MkdirReq
	58,  // 105: rpcpb.SliverRPC.Download:input_type -> sliverpb.DownloadReq
	59,  // 106: rpcpb.SliverRPC.Upload:input_type -> sliverpb.UploadReq
	60,  // 107: rpcpb.SliverRPC.Grep:input_type -> sliverpb.GrepReq
	61,  // 108: rpcpb.SliverRPC.Chmod:input_type -> sliverpb.ChmodReq
	62,  // 109: rpcpb.SliverRPC.Chown:input_type -> sliverpb.ChownReq
	63,  // 110: rpcpb.SliverRPC.Chtimes:input_type -> sliverpb.ChtimesReq
	64,  // 111: rpcpb.SliverRPC.MemfilesList:input_type -> sliverpb.MemfilesListReq
	65,  // 112: rpcpb.SliverRPC.MemfilesAdd:input_type -> sliverpb.MemfilesAddReq
	66,  // 113: rpcpb.SliverRPC.MemfilesRm:input_type -> sliverpb.MemfilesRmReq
	67,  // 114: rpcpb.SliverRPC.Mount:input_type -> sliverpb.MountReq
	68,  // 115: rpcpb.SliverRPC.ProcessDump:input_type -> sliverpb.ProcessDumpReq
	69,  // 116: rpcpb.SliverRPC.RunAs:input_type -> sliverpb.RunAsReq
	70,  // 117: rpcpb.SliverRPC.Impersonate:input_type -> sliverpb.ImpersonateReq
	71,  // 118: rpcpb.SliverRPC.RevToSelf:input_type -> sliverpb.RevToSelfReq
	72,  // 119: rpcpb.SliverRPC.GetSystem:input_type -> clientpb.GetSystemReq
	73,  // 120: rpcpb.SliverRPC.Task:input_type -> sliverpb.TaskReq
	74,  // 121: rpcpb.SliverRPC.Msf:input_type -> clientpb.MSFReq
	75,  // 122: rpcpb.SliverRPC.MsfRemote:input_type -> clientpb.MSFRemoteReq
	76,  // 123: rpcpb.SliverRPC.ExecuteAssembly:input_type -> sliverpb.ExecuteAssemblyReq
	77,  // 124: rpcpb.SliverRPC.Migrate:input_type -> clientpb.MigrateReq
	78,  // 125: rpcpb.SliverRPC.Execute:input_type -> sliverpb.ExecuteReq
	79,  // 126: rpcpb.SliverRPC.ExecuteWindows:input_type -> sliverpb.ExecuteWindowsReq
	80,  // 127: rpcpb.SliverRPC.ExecuteChildren:input_type -> sliverpb.ExecuteChildrenReq
	81,  // 128: rpcpb.SliverRPC.Sideload:input_type -> sliverpb.SideloadReq
	82,  // 129: rpcpb.SliverRPC.SpawnDll:input_type -> sliverpb.InvokeSpawnDllReq
	83,  // 130: rpcpb.SliverRPC.Screenshot:input_type -> sliverpb.ScreenshotReq
	84,  // 131: rpcpb.SliverRPC.CurrentTokenOwner:input_type -> sliverpb.CurrentTokenOwnerReq
	85,  // 132: rpcpb.SliverRPC.Services:input_type -> sliverpb.ServicesReq
	86,  // 133: rpcpb.SliverRPC.ServiceDetail:input_type -> sliverpb.ServiceDetailReq
	87,  // 134: rpcpb.SliverRPC.StartServiceByName:input_type -> sliverpb.StartServiceByNameReq
	88,  // 135: rpcpb.SliverRPC.PivotStartListener:input_type -> sliverpb.PivotStartListenerReq
	89,  // 136: rpcpb.SliverRPC.PivotStopListener:input_type -> sliverpb.PivotStopListenerReq
	90,  // 137: rpcpb.SliverRPC.PivotSessionListeners:input_type -> sliverpb.PivotListenersReq
	0,   // 138: rpcpb.SliverRPC.PivotGraph:input_type -> commonpb.Empty
	91,  // 139: rpcpb.SliverRPC.StartService:input_type -> sliverpb.StartServiceReq
	92,  // 140: rpcpb.SliverRPC.StopService:input_type -> sliverpb.StopServiceReq
	93,  // 141: rpcpb.SliverRPC.RemoveService:input_type -> sliverpb.RemoveServiceReq
	94,  // 142: rpcpb.SliverRPC.MakeToken:input_type -> sliverpb.MakeTokenReq
	95,  // 143: rpcpb.SliverRPC.GetEnv:input_type -> sliverpb.EnvReq
	96,  // 144: rpcpb.SliverRPC.SetEnv:input_type -> sliverpb.SetEnvReq
	97,  // 145: rpcpb.SliverRPC.UnsetEnv:input_type -> sliverpb.UnsetEnvReq
	98,  // 146: rpcpb.SliverRPC.Backdoor:input_type -> clientpb.BackdoorReq
	99,  // 147: rpcpb.SliverRPC.RegistryRead:input_type -> sliverpb.RegistryReadReq
	100, // 148: rpcpb.SliverRPC.RegistryWrite:input_type -> sliverpb.RegistryWriteReq
	101, // 149: rpcpb.SliverRPC.RegistryCreateKey:input_type -> sliverpb.RegistryCreateKeyReq
	102, // 150: rpcpb.SliverRPC.RegistryDeleteKey:input_type -> sliverpb.RegistryDeleteKeyReq
	103, // 151: rpcpb.SliverRPC.RegistryListSubKeys:input_type -> sliverpb.RegistrySubKeyListReq
	104, // 152: rpcpb.SliverRPC.RegistryListValues:input_type -> sliverpb.RegistryListValuesReq
	105, // 153: rpcpb.SliverRPC.RegistryReadHive:input_type -> sliverpb.RegistryReadHiveReq
	106, // 154: rpcpb.SliverRPC.RunSSHCommand:input_type -> sliverpb.SSHCommandReq
	107, // 155: rpcpb.SliverRPC.HijackDLL:input_type -> clientpb.DllHijackReq
	108, // 156: rpcpb.SliverRPC.GetPrivs:input_type -> sliverpb.GetPrivsReq
	109, // 157: rpcpb.SliverRPC.StartRportFwdListener:input_type -> sliverpb.RportFwdStartListenerReq
	110, // 158: rpcpb.SliverRPC.GetRportFwdListeners:input_type -> sliverpb.RportFwdListenersReq
	111, // 159: rpcpb.SliverRPC.StopRportFwdListener:input_type -> sliverpb.RportFwdStopListenerReq
	112, // 160: rpcpb.SliverRPC.OpenSession:input_type -> sliverpb.OpenSession
	113, // 161: rpcpb.SliverRPC.CloseSession:input_type -> sliverpb.CloseSession
	114, // 162: rpcpb.SliverRPC.RegisterExtension:input_type -> sliverpb.RegisterExtensionReq
	115, // 163: rpcpb.SliverRPC.CallExtension:input_type -> sliverpb.CallExtensionReq
	116, // 164: rpcpb.SliverRPC.ListExtensions:input_type -> sliverpb.ListExtensionsReq
	117, // 165: rpcpb.SliverRPC.RegisterWasmExtension:input_type -> sliverpb.RegisterWasmExtensionReq
	118, // 166: rpcpb.SliverRPC.ListWasmExtensions:input_type -> sliverpb.ListWasmExtensionsReq
	119, // 167: rpcpb.SliverRPC.ExecWasmExtension:input_type -> sliverpb.ExecWasmExtensionReq
	120, // 168: rpcpb.SliverRPC.WGStartPortForward:input_type -> sliverpb.WGPortForwardStartReq
	121, // 169: rpcpb.SliverRPC.WGStopPortForward:input_type -> sliverpb.WGPortForwardStopReq
	122, // 170: rpcpb.SliverRPC.WGStartSocks:input_type -> sliverpb.WGSocksStartReq
	123, // 171: rpcpb.SliverRPC.WGStopSocks:input_type -> sliverpb.WGSocksStopReq
	124, // 172: rpcpb.SliverRPC.WGListForwarders:input_type -> sliverpb.WGTCPForwardersReq
	125, // 173: rpcpb.SliverRPC.WGListSocksServers:input_type -> sliverpb.WGSocksServersReq
	126, // 174: rpcpb.SliverRPC.Shell:input_type -> sliverpb.ShellReq
	127, // 175: rpcpb.SliverRPC.ShellResize:input_type -> sliverpb.ShellResizeReq
	128, // 176: rpcpb.SliverRPC.Portfwd:input_type -> sliverpb.PortfwdReq
	129, // 177: rpcpb.SliverRPC.CreateSocks:input_type -> sliverpb.Socks
	129, // 178: rpcpb.SliverRPC.CloseSocks:input_type -> sliverpb.Socks
	130, // 179: rpcpb.SliverRPC.SocksProxy:input_type -> sliverpb.SocksData
	131, // 180: rpcpb.SliverRPC.CreateTunnel:input_type -> sliverpb.Tunnel
	131, // 181: rpcpb.SliverRPC.CloseTunnel:input_type -> sliverpb.Tunnel
	132, // 182: rpcpb.SliverRPC.TunnelData:input_type -> sliverpb.TunnelData
	0,   // 183: rpcpb.SliverRPC.Events:input_type -> commonpb.Empty
	133, // 184: rpcpb.SliverRPC.GetVersion:output_type -> clientpb.Version
	0,   // 185: rpcpb.SliverRPC.ClientLog:output_type -> commonpb.Empty
	134, // 186: rpcpb.SliverRPC.GetOperators:output_type -> clientpb.Operators
	0,   // 187: rpcpb.SliverRPC.Kill:output_type -> commonpb.Empty
	135, // 188: rpcpb.SliverRPC.Reconfigure:output_type -> sliverpb.Reconfigure
	0,   // 189: rpcpb.SliverRPC.Rename:output_type -> commonpb.Empty
	136, // 190: rpcpb.SliverRPC.GetSessions:output_type -> clientpb.Sessions
	137, // 191: rpcpb.SliverRPC.MonitorStart:output_type -> commonpb.Response
	0,   // 192: rpcpb.SliverRPC.MonitorStop:output_type -> commonpb.Empty
	138, // 193: rpcpb.SliverRPC.MonitorListConfig:output_type -> clientpb.MonitoringProviders
	137, // 194: rpcpb.SliverRPC.MonitorAddConfig:output_type -> commonpb.Response
	137, // 195: rpcpb.SliverRPC.MonitorDelConfig:output_type -> commonpb.Response
	139, // 196: rpcpb.SliverRPC.StartMTLSListener:output_type -> clientpb.ListenerJob
	139, // 197: rpcpb.SliverRPC.StartWGListener:output_type -> clientpb.ListenerJob
	139, // 198: rpcpb.SliverRPC.StartDNSListener:output_type -> clientpb.ListenerJob
	139, // 199: rpcpb.SliverRPC.StartHTTPSListener:output_type -> clientpb.ListenerJob
	139, // 200: rpcpb.SliverRPC.StartHTTPListener:output_type -> clientpb.ListenerJob
	140, // 201: rpcpb.SliverRPC.GetBeacons:output_type -> clientpb.Beacons
	10,  // 202: rpcpb.SliverRPC.GetBeacon:output_type -> clientpb.Beacon
	0,   // 203: rpcpb.SliverRPC.RmBeacon:output_type -> commonpb.Empty
	141, // 204: rpcpb.SliverRPC.GetBeaconTasks:output_type -> clientpb.BeaconTasks
	11,  // 205: rpcpb.SliverRPC.GetBeaconTaskContent:output_type -> clientpb.BeaconTask
	11,  // 206: rpcpb.SliverRPC.CancelBeaconTask:output_type -> clientpb.BeaconTask
	0,   // 207: rpcpb.SliverRPC.UpdateBeaconIntegrityInformation:output_type -> commonpb.Empty
	142, // 208: rpcpb.SliverRPC.GetJobs:output_type -> clientpb.Jobs
	143, // 209: rpcpb.SliverRPC.KillJob:output_type -> clientpb.KillJob
	0,   // 210: rpcpb.SliverRPC.RestartJobs:output_type -> commonpb.Empty
	144, // 211: rpcpb.SliverRPC.StartTCPStagerListener:output_type -> clientpb.StagerListener
	16,  // 212: rpcpb.SliverRPC.LootAdd:output_type -> clientpb.Loot
	0,   // 213: rpcpb.SliverRPC.LootRm:output_type -> commonpb.Empty
	16,  // 214: rpcpb.SliverRPC.LootUpdate:output_type -> clientpb.Loot
	16,  // 215: rpcpb.SliverRPC.LootContent:output_type -> clientpb.Loot
	145, // 216: rpcpb.SliverRPC.LootAll:output_type -> clientpb.AllLoot
	17,  // 217: rpcpb.SliverRPC.Creds:output_type -> clientpb.Credentials
	0,   // 218: rpcpb.SliverRPC.CredsAdd:output_type -> commonpb.Empty
	0,   // 219: rpcpb.SliverRPC.CredsRm:output_type -> commonpb.Empty
	0,   // 220: rpcpb.SliverRPC.CredsUpdate:output_type -> commonpb.Empty
	18,  // 221: rpcpb.SliverRPC.GetCredByID:output_type -> clientpb.Credential
	17,  // 222: rpcpb.SliverRPC.GetCredsByHashType:output_type -> clientpb.Credentials
	17,  // 223: rpcpb.SliverRPC.GetPlaintextCredsByHashType:output_type -> clientpb.Credentials
	18,  // 224: rpcpb.SliverRPC.CredsSniffHashType:output_type -> clientpb.Credential
	146, // 225: rpcpb.SliverRPC.Hosts:output_type -> clientpb.AllHosts
	19,  // 226: rpcpb.SliverRPC.Host:output_type -> clientpb.Host
	0,   // 227: rpcpb.SliverRPC.HostRm:output_type -> commonpb.Empty
	0,   // 228: rpcpb.SliverRPC.HostIOCRm:output_type -> commonpb.Empty
	147, // 229: rpcpb.SliverRPC.Generate:output_type -> clientpb.Generate
	148, // 230: rpcpb.SliverRPC.GenerateExternal:output_type -> clientpb.ExternalImplantConfig
	0,   // 231: rpcpb.SliverRPC.GenerateExternalSaveBuild:output_type -> commonpb.Empty
	148, // 232: rpcpb.SliverRPC.GenerateExternalGetBuildConfig:output_type -> clientpb.ExternalImplantConfig
	147, // 233: rpcpb.SliverRPC.GenerateStage:output_type -> clientpb.Generate
	0,   // 234: rpcpb.SliverRPC.StageImplantBuild:output_type -> commonpb.Empty
	149, // 235: rpcpb.SliverRPC.GetHTTPC2Profiles:output_type -> clientpb.HTTPC2Configs
	150, // 236: rpcpb.SliverRPC.GetHTTPC2ProfileByName:output_type -> clientpb.HTTPC2Config
	0,   // 237: rpcpb.SliverRPC.SaveHTTPC2Profile:output_type -> commonpb.Empty
	30,  // 238: rpcpb.SliverRPC.BuilderRegister:output_type -> clientpb.Event
	0,   // 239: rpcpb.SliverRPC.BuilderTrigger:output_type -> commonpb.Empty
	151, // 240: rpcpb.SliverRPC.Builders:output_type -> clientpb.Builders
	152, // 241: rpcpb.SliverRPC.GetCertificateInfo:output_type -> clientpb.CertificateInfo
	153, // 242: rpcpb.SliverRPC.GetCertificateAuthorityInfo:output_type -> clientpb.CertificateAuthorityInfo
	30,  // 243: rpcpb.SliverRPC.CrackstationRegister:output_type -> clientpb.Event
	0,   // 244: rpcpb.SliverRPC.CrackstationTrigger:output_type -> commonpb.Empty
	0,   // 245: rpcpb.SliverRPC.CrackstationBenchmark:output_type -> commonpb.Empty
	154, // 246: rpcpb.SliverRPC.Crackstations:output_type -> clientpb.Crackstations
	34,  // 247: rpcpb.SliverRPC.CrackTaskByID:output_type -> clientpb.CrackTask
	0,   // 248: rpcpb.SliverRPC.CrackTaskUpdate:output_type -> commonpb.Empty
	155, // 249: rpcpb.SliverRPC.CrackFilesList:output_type -> clientpb.CrackFiles
	35,  // 250: rpcpb.SliverRPC.CrackFileCreate:output_type -> clientpb.CrackFile
	0,   // 251: rpcpb.SliverRPC.CrackFileChunkUpload:output_type -> commonpb.Empty
	36,  // 252: rpcpb.SliverRPC.CrackFileChunkDownload:output_type -> clientpb.CrackFileChunk
	0,   // 253: rpcpb.SliverRPC.CrackFileComplete:output_type -> commonpb.Empty
	0,   // 254: rpcpb.SliverRPC.CrackFileDelete:output_type -> commonpb.Empty
	147, // 255: rpcpb.SliverRPC.Regenerate:output_type -> clientpb.Generate
	156, // 256: rpcpb.SliverRPC.ImplantBuilds:output_type -> clientpb.ImplantBuilds
	0,   // 257: rpcpb.SliverRPC.DeleteImplantBuild:output_type -> commonpb.Empty
	157, // 258: rpcpb.SliverRPC.Canaries:output_type -> clientpb.Canaries
	158, // 259: rpcpb.SliverRPC.GenerateWGClientConfig:output_type -> clientpb.WGClientConfig
	159, // 260: rpcpb.SliverRPC.GenerateUniqueIP:output_type -> clientpb.UniqueWGIP
	160, // 261: rpcpb.SliverRPC.ImplantProfiles:output_type -> clientpb.ImplantProfiles
	0,   // 262: rpcpb.SliverRPC.DeleteImplantProfile:output_type -> commonpb.Empty
	39,  // 263: rpcpb.SliverRPC.SaveImplantProfile:output_type -> clientpb.ImplantProfile
	161, // 264: rpcpb.SliverRPC.ShellcodeRDI:output_type -> clientpb.ShellcodeRDI
	162, // 265: rpcpb.SliverRPC.GetCompiler:output_type -> clientpb.Compiler
	163, // 266: rpcpb.SliverRPC.ShellcodeEncoder:output_type -> clientpb.ShellcodeEncode
	164, // 267: rpcpb.SliverRPC.ShellcodeEncoderMap:output_type -> clientpb.ShellcodeEncoderMap
	165, // 268: rpcpb.SliverRPC.TrafficEncoderMap:output_type -> clientpb.TrafficEncoderMap
	166, // 269: rpcpb.SliverRPC.TrafficEncoderAdd:output_type -> clientpb.TrafficEncoderTests
	0,   // 270: rpcpb.SliverRPC.TrafficEncoderRm:output_type -> commonpb.Empty
	167, // 271: rpcpb.SliverRPC.Websites:output_type -> clientpb.Websites
	43,  // 272: rpcpb.SliverRPC.Website:output_type -> clientpb.Website
	0,   // 273: rpcpb.SliverRPC.WebsiteRemove:output_type -> commonpb.Empty
	43,  // 274: rpcpb.SliverRPC.WebsiteAddContent:output_type -> clientpb.Website
	43,  // 275: rpcpb.SliverRPC.WebsiteUpdateContent:output_type -> clientpb.Website
	43,  // 276: rpcpb.SliverRPC.WebsiteRemoveContent:output_type -> clientpb.Website
	46,  // 277: rpcpb.SliverRPC.Ping:output_type -> sliverpb.Ping
	168, // 278: rpcpb.SliverRPC.Ps:output_type -> sliverpb.Ps
	169, // 279: rpcpb.SliverRPC.Terminate:output_type -> sliverpb.Terminate
	170, // 280: rpcpb.SliverRPC.Ifconfig:output_type -> sliverpb.Ifconfig
	171, // 281: rpcpb.SliverRPC.Netstat:output_type -> sliverpb.Netstat
	172, // 282: rpcpb.SliverRPC.Ls:output_type -> sliverpb.Ls
	173, // 283: rpcpb.SliverRPC.Cd:output_type -> sliverpb.Pwd
	173, // 284: rpcpb.SliverRPC.Pwd:output_type -> sliverpb.Pwd
	174, // 285: rpcpb.SliverRPC.Mv:output_type -> sliverpb.Mv
	175, // 286: rpcpb.SliverRPC.Cp:output_type -> sliverpb.Cp
	176, // 287: rpcpb.SliverRPC.Rm:output_type -> sliverpb.Rm
	177, // 288: rpcpb.SliverRPC.Mkdir:output_type -> sliverpb.Mkdir
	178, // 289: rpcpb.SliverRPC.Download:output_type -> sliverpb.Download
	179, // 290: rpcpb.SliverRPC.Upload:output_type -> sliverpb.Upload
	180, // 291: rpcpb.SliverRPC.Grep:output_type -> sliverpb.Grep
	181, // 292: rpcpb.SliverRPC.Chmod:output_type -> sliverpb.Chmod
	182, // 293: rpcpb.SliverRPC.Chown:output_type -> sliverpb.Chown
	183, // 294: rpcpb.SliverRPC.Chtimes:output_type -> sliverpb.Chtimes
	172, // 295: rpcpb.SliverRPC.MemfilesList:output_type -> sliverpb.Ls
	184, // 296: rpcpb.SliverRPC.MemfilesAdd:output_type -> sliverpb.MemfilesAdd
	185, // 297: rpcpb.SliverRPC.MemfilesRm:output_type -> sliverpb.MemfilesRm
	186, // 298: rpcpb.SliverRPC.Mount:output_type -> sliverpb.Mount
	187, // 299: rpcpb.SliverRPC.ProcessDump:output_type -> sliverpb.ProcessDump
	188, // 300: rpcpb.SliverRPC.RunAs:output_type -> sliverpb.RunAs
	189, // 301: rpcpb.SliverRPC.Impersonate:output_type -> sliverpb.Impersonate
	190, // 302: rpcpb.SliverRPC.RevToSelf:output_type -> sliverpb.RevToSelf
	191, // 303: rpcpb.SliverRPC.GetSystem:output_type -> sliverpb.GetSystem
	192, // 304: rpcpb.SliverRPC.Task:output_type -> sliverpb.Task
	192, // 305: rpcpb.SliverRPC.Msf:output_type -> sliverpb.Task
	192, // 306: rpcpb.SliverRPC.MsfRemote:output_type -> sliverpb.Task
	193, // 307: rpcpb.SliverRPC.ExecuteAssembly:output_type -> sliverpb.ExecuteAssembly
	194, // 308: rpcpb.SliverRPC.Migrate:output_type -> sliverpb.Migrate
	195, // 309: rpcpb.SliverRPC.Execute:output_type -> sliverpb.Execute
	195, // 310: rpcpb.SliverRPC.ExecuteWindows:output_type -> sliverpb.Execute
	196, // 311: rpcpb.SliverRPC.ExecuteChildren:output_type -> sliverpb.ExecuteChildren
	197, // 312: rpcpb.SliverRPC.Sideload:output_type -> sliverpb.Sideload
	198, // 313: rpcpb.SliverRPC.SpawnDll:output_type -> sliverpb.SpawnDll
	199, // 314: rpcpb.SliverRPC.Screenshot:output_type -> sliverpb.Screenshot
	200, // 315: rpcpb.SliverRPC.CurrentTokenOwner:output_type -> sliverpb.CurrentTokenOwner
	201, // 316: rpcpb.SliverRPC.Services:output_type -> sliverpb.Services
	202, // 317: rpcpb.SliverRPC.ServiceDetail:output_type -> sliverpb.ServiceDetail
	203, // 318: rpcpb.SliverRPC.StartServiceByName:output_type -> sliverpb.ServiceInfo
	204, // 319: rpcpb.SliverRPC.PivotStartListener:output_type -> sliverpb.PivotListener
	0,   // 320: rpcpb.SliverRPC.PivotStopListener:output_type -> commonpb.Empty
	205, // 321: rpcpb.SliverRPC.PivotSessionListeners:output_type -> sliverpb.PivotListeners
	206, // 322: rpcpb.SliverRPC.PivotGraph:output_type -> clientpb.PivotGraph
	203, // 323: rpcpb.SliverRPC.StartService:output_type -> sliverpb.ServiceInfo
	203, // 324: rpcpb.SliverRPC.StopService:output_type -> sliverpb.ServiceInfo
	203, // 325: rpcpb.SliverRPC.RemoveService:output_type -> sliverpb.ServiceInfo
	207, // 326: rpcpb.SliverRPC.MakeToken:output_type -> sliverpb.MakeToken
	208, // 327: rpcpb.SliverRPC.GetEnv:output_type -> sliverpb.EnvInfo
	209, // 328: rpcpb.SliverRPC.SetEnv:output_type -> sliverpb.SetEnv
	210, // 329: rpcpb.SliverRPC.UnsetEnv:output_type -> sliverpb.UnsetEnv
	211, // 330: rpcpb.SliverRPC.Backdoor:output_type -> clientpb.Backdoor
	212, // 331: rpcpb.SliverRPC.RegistryRead:output_type -> sliverpb.RegistryRead
	213, // 332: rpcpb.SliverRPC.RegistryWrite:output_type -> sliverpb.RegistryWrite
	214, // 333: rpcpb.SliverRPC.RegistryCreateKey:output_type -> sliverpb.RegistryCreateKey
	215, // 334: rpcpb.SliverRPC.RegistryDeleteKey:output_type -> sliverpb.RegistryDeleteKey
	216, // 335: rpcpb.SliverRPC.RegistryListSubKeys:output_type -> sliverpb.RegistrySubKeyList
	217, // 336: rpcpb.SliverRPC.RegistryListValues:output_type -> sliverpb.RegistryValuesList
	218, // 337: rpcpb.SliverRPC.RegistryReadHive:output_type -> sliverpb.RegistryReadHive
	219, // 338: rpcpb.SliverRPC.RunSSHCommand:output_type -> sliverpb.SSHCommand
	220, // 339: rpcpb.SliverRPC.HijackDLL:output_type -> clientpb.DllHijack
	221, // 340: rpcpb.SliverRPC.GetPrivs:output_type -> sliverpb.GetPrivs
	222, // 341: rpcpb.SliverRPC.StartRportFwdListener:output_type -> sliverpb.RportFwdListener
	223, // 342: rpcpb.SliverRPC.GetRportFwdListeners:output_type -> sliverpb.RportFwdListeners
	222, // 343: rpcpb.SliverRPC.StopRportFwdListener:output_type -> sliverpb.RportFwdListener
	112, // 344: rpcpb.SliverRPC.OpenSession:output_type -> sliverpb.OpenSession
	0,   // 345: rpcpb.SliverRPC.CloseSession:output_type -> commonpb.Empty
	224, // 346: rpcpb.SliverRPC.RegisterExtension:output_type -> sliverpb.RegisterExtension
	225, // 347: rpcpb.SliverRPC.CallExtension:output_type -> sliverpb.CallExtension
	226, // 348: rpcpb.SliverRPC.ListExtensions:output_type -> sliverpb.ListExtensions
	227, // 349: rpcpb.SliverRPC.RegisterWasmExtension:output_type -> sliverpb.RegisterWasmExtension
	228, // 350: rpcpb.SliverRPC.ListWasmExtensions:output_type -> sliverpb.ListWasmExtensions
	229, // 351: rpcpb.SliverRPC.ExecWasmExtension:output_type -> sliverpb.ExecWasmExtension
	230, // 352: rpcpb.SliverRPC.WGStartPortForward:output_type -> sliverpb.WGPortForward
	230, // 353: rpcpb.SliverRPC.WGStopPortForward:output_type -> sliverpb.WGPortForward
	231, // 354: rpcpb.SliverRPC.WGStartSocks:output_type -> sliverpb.WGSocks
	231, // 355: rpcpb.SliverRPC.WGStopSocks:output_type -> sliverpb.WGSocks
	232, // 356: rpcpb.SliverRPC.WGListForwarders:output_type -> sliverpb.WGTCPForwarders
	233, // 357: rpcpb.SliverRPC.WGListSocksServers:output_type -> sliverpb.WGSocksServers
	234, // 358: rpcpb.SliverRPC.Shell:output_type -> sliverpb.Shell
	0,   // 359: rpcpb.SliverRPC.ShellResize:output_type -> commonpb.Empty
	235, // 360: rpcpb.SliverRPC.Portfwd:output_type -> sliverpb.Portfwd
	129, // 361: rpcpb.SliverRPC.CreateSocks:output_type -> sliverpb.Socks
	0,   // 362: rpcpb.SliverRPC.CloseSocks:output_type -> commonpb.Empty
	130, // 363: rpcpb.SliverRPC.SocksProxy:output_type -> sliverpb.SocksData
	131, // 364: rpcpb.SliverRPC.CreateTunnel:output_type -> sliverpb.Tunnel
	0,   // 365: rpcpb.SliverRPC.CloseTunnel:output_type -> commonpb.Empty
	132, // 366: rpcpb.SliverRPC.TunnelData:output_type -> sliverpb.TunnelData
	30,  // 367: rpcpb.SliverRPC.Events:output_type -> clientpb.Event
	184, // [184:368] is the sub-list for method output_type
	0,   // [0:184] is the sub-list for method input_type
	0,   // [0:0] is the sub-list for extension type_name
	0,   // [0:0] is the sub-list for extension extendee
	0,   // [0:0] is the sub-list for field type_name
}

func init() { file_rpcpb_services_proto_init() }
func file_rpcpb_services_proto_init() {
	if File_rpcpb_services_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_rpcpb_services_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_rpcpb_services_proto_goTypes,
		DependencyIndexes: file_rpcpb_services_proto_depIdxs,
	}.Build()
	File_rpcpb_services_proto = out.File
	file_rpcpb_services_proto_rawDesc = nil
	file_rpcpb_services_proto_goTypes = nil
	file_rpcpb_services_proto_depIdxs = nil
}
