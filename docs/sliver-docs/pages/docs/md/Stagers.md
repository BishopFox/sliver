...
- stagers (generated by `msfvenom` or built by hand, see [Custom Stagers](#custom-stagers) below)
...
> **Note:** The Sliver `generate stager` command has been removed and will no longer
> work. Generate stagers using Metasploit's `msfvenom` (or write your own custom
> stager, see [Custom Stagers](#custom-stagers) below).

#### Generating an HTTP(S) stager
