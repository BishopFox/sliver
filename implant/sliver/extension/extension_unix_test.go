...
func TestUnixExtensionCallbackCopiesOutput(t *testing.T) {
	...
	result := extension.extensionCallback(unsafe.Pointer(&source[0]), int32(len(source)))
	if result != Success {
...
func TestUnixExtensionCallbackRejectsInvalidCalls(t *testing.T) {
	extension := &UnixExtension{}
	if result := extension.extensionCallback(nil, 1); result != Failure {
	...
	source := byte(0)
	if result := extension.extensionCallback(unsafe.Pointer(&source), -1); result != Failure {
...
func TestUnixExtensionCallbackContainsPanics(t *testing.T) {
	...
	source := []byte("output")
	if result := extension.extensionCallback(unsafe.Pointer(&source[0]), int32(len(source))); result != Failure {
