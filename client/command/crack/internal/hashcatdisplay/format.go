...
// TypeName returns the display name for a Hashcat mode.
func TypeName(mode int32) (string, bool) {
	name, ok := hashcatHashTypes[mode]
	return name, ok
}

// HumanizeRate formats a hashes-per-second rate for terminal display.
func HumanizeRate(rate uint64) string {
	const unit = 1000
	if rate < unit {
