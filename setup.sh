...
# ─── start.sh ───
cat > "$SLIVER_DIR/start.sh" << 'EOF'
#!/bin/bash
# Start Sliver server. On first run, prints profile import command.
...
if [ ! -f "$MARKER" ]; then
    echo ""
    echo "[*] First run — after the console loads, import the C2 profile:"
    echo ""
    echo "    c2profiles import $PROFILE --name microsoft365"
    echo ""
    echo "[*] Then start listeners:"
    echo ""
    echo "    mtls -l 0.0.0.0 -p 8888"
    echo "    https -l 0.0.0.0 -p 443 -d your-domain.com"
    echo ""
    mkdir -p "$HOME/.sliver" 2>/dev/null
    touch "$MARKER"
fi

exec "$SCRIPT_DIR/sliver-server"
EOF
...
echo -e "     https -l 0.0.0.0 -p 443 -d your-domain.com"
echo ""
