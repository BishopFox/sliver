...
func (w *writer) codeFromTree(tree *RegexTree) (*Code, error) {
	...
	var fcPrefix *Prefix
	var prefix *Prefix
	if !tree.root.hasOptions(customCaseOptions) {
...
func (w *writer) emitFragment(nodetype nodeType, node *regexNode, curIndex int) error {
	...
	if nodetype <= ntRef || nodetype == ntGrapheme {
		if (node.options & RightToLeft) != 0 {
			bits |= Rtl
		}
		if (node.options & JavaASCII) != 0 {
			bits |= CiASCII
		} else if (node.options & JavaUnicode) != 0 {
			bits |= CiJava
		} else if (node.options & IgnoreCase) != 0 {
			bits |= Ci
		}
	}
	...
	case ntGrapheme:
		w.emit(Grapheme | bits)

	case ntNothing, ntBol, ntEol, ntBoundary, ntNonboundary, ntECMABoundary, ntNonECMABoundary,
		ntGraphemeBoundary, ntBeginning, ntStart, ntEndZ, ntEnd:
