// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v3.19.3
// source: rpcpb/services.proto

package rpcpb

import (
	clientpb "github.com/bishopfox/sliver/protobuf/clientpb"
	commonpb "github.com/bishopfox/sliver/protobuf/commonpb"
	sliverpb "github.com/bishopfox/sliver/protobuf/sliverpb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_rpcpb_services_proto protoreflect.FileDescriptor

var file_rpcpb_services_proto_rawDesc = []byte{
	,
}

var file_rpcpb_services_proto_goTypes = []interface{}{
	(*commonpb.Empty)(nil),                 // 0: commonpb.Empty
	(*sliverpb.KillReq)(nil),               // 1: sliverpb.KillReq
	(*sliverpb.ReconfigureReq)(nil),        // 2: sliverpb.ReconfigureReq
	(*clientpb.RenameReq)(nil),             // 3: clientpb.RenameReq
	(*clientpb.Beacon)(nil),                // 4: clientpb.Beacon
	(*clientpb.BeaconTask)(nil),            // 5: clientpb.BeaconTask
	(*clientpb.KillJobReq)(nil),            // 6: clientpb.KillJobReq
	(*clientpb.MTLSListenerReq)(nil),       // 7: clientpb.MTLSListenerReq
	(*clientpb.WGListenerReq)(nil),         // 8: clientpb.WGListenerReq
	(*clientpb.DNSListenerReq)(nil),        // 9: clientpb.DNSListenerReq
	(*clientpb.HTTPListenerReq)(nil),       // 10: clientpb.HTTPListenerReq
	(*clientpb.StagerListenerReq)(nil),     // 11: clientpb.StagerListenerReq
	(*clientpb.Loot)(nil),                  // 12: clientpb.Loot
	(*clientpb.Host)(nil),                  // 13: clientpb.Host
	(*clientpb.IOC)(nil),                   // 14: clientpb.IOC
	(*clientpb.GenerateReq)(nil),           // 15: clientpb.GenerateReq
	(*clientpb.RegenerateReq)(nil),         // 16: clientpb.RegenerateReq
	(*clientpb.DeleteReq)(nil),             // 17: clientpb.DeleteReq
	(*clientpb.ImplantProfile)(nil),        // 18: clientpb.ImplantProfile
	(*clientpb.MsfStagerReq)(nil),          // 19: clientpb.MsfStagerReq
	(*clientpb.ShellcodeRDIReq)(nil),       // 20: clientpb.ShellcodeRDIReq
	(*clientpb.Website)(nil),               // 21: clientpb.Website
	(*clientpb.WebsiteAddContent)(nil),     // 22: clientpb.WebsiteAddContent
	(*clientpb.WebsiteRemoveContent)(nil),  // 23: clientpb.WebsiteRemoveContent
	(*sliverpb.Ping)(nil),                  // 24: sliverpb.Ping
	(*sliverpb.PsReq)(nil),                 // 25: sliverpb.PsReq
	(*sliverpb.TerminateReq)(nil),          // 26: sliverpb.TerminateReq
	(*sliverpb.IfconfigReq)(nil),           // 27: sliverpb.IfconfigReq
	(*sliverpb.NetstatReq)(nil),            // 28: sliverpb.NetstatReq
	(*sliverpb.LsReq)(nil),                 // 29: sliverpb.LsReq
	(*sliverpb.CdReq)(nil),                 // 30: sliverpb.CdReq
	(*sliverpb.PwdReq)(nil),                // 31: sliverpb.PwdReq
	(*sliverpb.RmReq)(nil),                 // 32: sliverpb.RmReq
	(*sliverpb.MkdirReq)(nil),              // 33: sliverpb.MkdirReq
	(*sliverpb.DownloadReq)(nil),           // 34: sliverpb.DownloadReq
	(*sliverpb.UploadReq)(nil),             // 35: sliverpb.UploadReq
	(*sliverpb.ProcessDumpReq)(nil),        // 36: sliverpb.ProcessDumpReq
	(*sliverpb.RunAsReq)(nil),              // 37: sliverpb.RunAsReq
	(*sliverpb.ImpersonateReq)(nil),        // 38: sliverpb.ImpersonateReq
	(*sliverpb.RevToSelfReq)(nil),          // 39: sliverpb.RevToSelfReq
	(*clientpb.GetSystemReq)(nil),          // 40: clientpb.GetSystemReq
	(*sliverpb.TaskReq)(nil),               // 41: sliverpb.TaskReq
	(*clientpb.MSFReq)(nil),                // 42: clientpb.MSFReq
	(*clientpb.MSFRemoteReq)(nil),          // 43: clientpb.MSFRemoteReq
	(*sliverpb.ExecuteAssemblyReq)(nil),    // 44: sliverpb.ExecuteAssemblyReq
	(*clientpb.MigrateReq)(nil),            // 45: clientpb.MigrateReq
	(*sliverpb.ExecuteReq)(nil),            // 46: sliverpb.ExecuteReq
	(*sliverpb.ExecuteTokenReq)(nil),       // 47: sliverpb.ExecuteTokenReq
	(*sliverpb.SideloadReq)(nil),           // 48: sliverpb.SideloadReq
	(*sliverpb.InvokeSpawnDllReq)(nil),     // 49: sliverpb.InvokeSpawnDllReq
	(*sliverpb.ScreenshotReq)(nil),         // 50: sliverpb.ScreenshotReq
	(*sliverpb.PivotStartListenerReq)(nil), // 51: sliverpb.PivotStartListenerReq
	(*sliverpb.PivotStopListenerReq)(nil),  // 52: sliverpb.PivotStopListenerReq
	(*sliverpb.PivotListenersReq)(nil),     // 53: sliverpb.PivotListenersReq
	(*sliverpb.StartServiceReq)(nil),       // 54: sliverpb.StartServiceReq
	(*sliverpb.StopServiceReq)(nil),        // 55: sliverpb.StopServiceReq
	(*sliverpb.RemoveServiceReq)(nil),      // 56: sliverpb.RemoveServiceReq
	(*sliverpb.MakeTokenReq)(nil),          // 57: sliverpb.MakeTokenReq
	(*sliverpb.EnvReq)(nil),                // 58: sliverpb.EnvReq
	(*sliverpb.SetEnvReq)(nil),             // 59: sliverpb.SetEnvReq
	(*sliverpb.UnsetEnvReq)(nil),           // 60: sliverpb.UnsetEnvReq
	(*sliverpb.BackdoorReq)(nil),           // 61: sliverpb.BackdoorReq
	(*sliverpb.RegistryReadReq)(nil),       // 62: sliverpb.RegistryReadReq
	(*sliverpb.RegistryWriteReq)(nil),      // 63: sliverpb.RegistryWriteReq
	(*sliverpb.RegistryCreateKeyReq)(nil),  // 64: sliverpb.RegistryCreateKeyReq
	(*sliverpb.RegistryDeleteKeyReq)(nil),  // 65: sliverpb.RegistryDeleteKeyReq
	(*sliverpb.RegistrySubKeyListReq)(nil), // 66: sliverpb.RegistrySubKeyListReq
	(*sliverpb.RegistryListValuesReq)(nil), // 67: sliverpb.RegistryListValuesReq
	(*sliverpb.SSHCommandReq)(nil),         // 68: sliverpb.SSHCommandReq
	(*clientpb.DllHijackReq)(nil),          // 69: clientpb.DllHijackReq
	(*sliverpb.GetPrivsReq)(nil),           // 70: sliverpb.GetPrivsReq
	(*sliverpb.OpenSession)(nil),           // 71: sliverpb.OpenSession
	(*sliverpb.CloseSession)(nil),          // 72: sliverpb.CloseSession
	(*sliverpb.RegisterExtensionReq)(nil),  // 73: sliverpb.RegisterExtensionReq
	(*sliverpb.CallExtensionReq)(nil),      // 74: sliverpb.CallExtensionReq
	(*sliverpb.ListExtensionsReq)(nil),     // 75: sliverpb.ListExtensionsReq
	(*sliverpb.WGPortForwardStartReq)(nil), // 76: sliverpb.WGPortForwardStartReq
	(*sliverpb.WGPortForwardStopReq)(nil),  // 77: sliverpb.WGPortForwardStopReq
	(*sliverpb.WGSocksStartReq)(nil),       // 78: sliverpb.WGSocksStartReq
	(*sliverpb.WGSocksStopReq)(nil),        // 79: sliverpb.WGSocksStopReq
	(*sliverpb.WGTCPForwardersReq)(nil),    // 80: sliverpb.WGTCPForwardersReq
	(*sliverpb.WGSocksServersReq)(nil),     // 81: sliverpb.WGSocksServersReq
	(*sliverpb.ShellReq)(nil),              // 82: sliverpb.ShellReq
	(*sliverpb.PortfwdReq)(nil),            // 83: sliverpb.PortfwdReq
	(*sliverpb.Socks)(nil),                 // 84: sliverpb.Socks
	(*sliverpb.SocksData)(nil),             // 85: sliverpb.SocksData
	(*sliverpb.Tunnel)(nil),                // 86: sliverpb.Tunnel
	(*sliverpb.TunnelData)(nil),            // 87: sliverpb.TunnelData
	(*clientpb.Version)(nil),               // 88: clientpb.Version
	(*clientpb.Operators)(nil),             // 89: clientpb.Operators
	(*sliverpb.Reconfigure)(nil),           // 90: sliverpb.Reconfigure
	(*clientpb.Sessions)(nil),              // 91: clientpb.Sessions
	(*clientpb.Beacons)(nil),               // 92: clientpb.Beacons
	(*clientpb.BeaconTasks)(nil),           // 93: clientpb.BeaconTasks
	(*commonpb.Response)(nil),              // 94: commonpb.Response
	(*clientpb.Jobs)(nil),                  // 95: clientpb.Jobs
	(*clientpb.KillJob)(nil),               // 96: clientpb.KillJob
	(*clientpb.MTLSListener)(nil),          // 97: clientpb.MTLSListener
	(*clientpb.WGListener)(nil),            // 98: clientpb.WGListener
	(*clientpb.DNSListener)(nil),           // 99: clientpb.DNSListener
	(*clientpb.HTTPListener)(nil),          // 100: clientpb.HTTPListener
	(*clientpb.StagerListener)(nil),        // 101: clientpb.StagerListener
	(*clientpb.AllLoot)(nil),               // 102: clientpb.AllLoot
	(*clientpb.AllHosts)(nil),              // 103: clientpb.AllHosts
	(*clientpb.Generate)(nil),              // 104: clientpb.Generate
	(*clientpb.ImplantBuilds)(nil),         // 105: clientpb.ImplantBuilds
	(*clientpb.Canaries)(nil),              // 106: clientpb.Canaries
	(*clientpb.WGClientConfig)(nil),        // 107: clientpb.WGClientConfig
	(*clientpb.UniqueWGIP)(nil),            // 108: clientpb.UniqueWGIP
	(*clientpb.ImplantProfiles)(nil),       // 109: clientpb.ImplantProfiles
	(*clientpb.MsfStager)(nil),             // 110: clientpb.MsfStager
	(*clientpb.ShellcodeRDI)(nil),          // 111: clientpb.ShellcodeRDI
	(*clientpb.Compiler)(nil),              // 112: clientpb.Compiler
	(*clientpb.Websites)(nil),              // 113: clientpb.Websites
	(*sliverpb.Ps)(nil),                    // 114: sliverpb.Ps
	(*sliverpb.Terminate)(nil),             // 115: sliverpb.Terminate
	(*sliverpb.Ifconfig)(nil),              // 116: sliverpb.Ifconfig
	(*sliverpb.Netstat)(nil),               // 117: sliverpb.Netstat
	(*sliverpb.Ls)(nil),                    // 118: sliverpb.Ls
	(*sliverpb.Pwd)(nil),                   // 119: sliverpb.Pwd
	(*sliverpb.Rm)(nil),                    // 120: sliverpb.Rm
	(*sliverpb.Mkdir)(nil),                 // 121: sliverpb.Mkdir
	(*sliverpb.Download)(nil),              // 122: sliverpb.Download
	(*sliverpb.Upload)(nil),                // 123: sliverpb.Upload
	(*sliverpb.ProcessDump)(nil),           // 124: sliverpb.ProcessDump
	(*sliverpb.RunAs)(nil),                 // 125: sliverpb.RunAs
	(*sliverpb.Impersonate)(nil),           // 126: sliverpb.Impersonate
	(*sliverpb.RevToSelf)(nil),             // 127: sliverpb.RevToSelf
	(*sliverpb.GetSystem)(nil),             // 128: sliverpb.GetSystem
	(*sliverpb.Task)(nil),                  // 129: sliverpb.Task
	(*sliverpb.ExecuteAssembly)(nil),       // 130: sliverpb.ExecuteAssembly
	(*sliverpb.Migrate)(nil),               // 131: sliverpb.Migrate
	(*sliverpb.Execute)(nil),               // 132: sliverpb.Execute
	(*sliverpb.Sideload)(nil),              // 133: sliverpb.Sideload
	(*sliverpb.SpawnDll)(nil),              // 134: sliverpb.SpawnDll
	(*sliverpb.Screenshot)(nil),            // 135: sliverpb.Screenshot
	(*sliverpb.PivotListener)(nil),         // 136: sliverpb.PivotListener
	(*sliverpb.PivotListeners)(nil),        // 137: sliverpb.PivotListeners
	(*clientpb.PivotGraph)(nil),            // 138: clientpb.PivotGraph
	(*sliverpb.ServiceInfo)(nil),           // 139: sliverpb.ServiceInfo
	(*sliverpb.MakeToken)(nil),             // 140: sliverpb.MakeToken
	(*sliverpb.EnvInfo)(nil),               // 141: sliverpb.EnvInfo
	(*sliverpb.SetEnv)(nil),                // 142: sliverpb.SetEnv
	(*sliverpb.UnsetEnv)(nil),              // 143: sliverpb.UnsetEnv
	(*sliverpb.Backdoor)(nil),              // 144: sliverpb.Backdoor
	(*sliverpb.RegistryRead)(nil),          // 145: sliverpb.RegistryRead
	(*sliverpb.RegistryWrite)(nil),         // 146: sliverpb.RegistryWrite
	(*sliverpb.RegistryCreateKey)(nil),     // 147: sliverpb.RegistryCreateKey
	(*sliverpb.RegistryDeleteKey)(nil),     // 148: sliverpb.RegistryDeleteKey
	(*sliverpb.RegistrySubKeyList)(nil),    // 149: sliverpb.RegistrySubKeyList
	(*sliverpb.RegistryValuesList)(nil),    // 150: sliverpb.RegistryValuesList
	(*sliverpb.SSHCommand)(nil),            // 151: sliverpb.SSHCommand
	(*clientpb.DllHijack)(nil),             // 152: clientpb.DllHijack
	(*sliverpb.GetPrivs)(nil),              // 153: sliverpb.GetPrivs
	(*sliverpb.RegisterExtension)(nil),     // 154: sliverpb.RegisterExtension
	(*sliverpb.CallExtension)(nil),         // 155: sliverpb.CallExtension
	(*sliverpb.ListExtensions)(nil),        // 156: sliverpb.ListExtensions
	(*sliverpb.WGPortForward)(nil),         // 157: sliverpb.WGPortForward
	(*sliverpb.WGSocks)(nil),               // 158: sliverpb.WGSocks
	(*sliverpb.WGTCPForwarders)(nil),       // 159: sliverpb.WGTCPForwarders
	(*sliverpb.WGSocksServers)(nil),        // 160: sliverpb.WGSocksServers
	(*sliverpb.Shell)(nil),                 // 161: sliverpb.Shell
	(*sliverpb.Portfwd)(nil),               // 162: sliverpb.Portfwd
	(*clientpb.Event)(nil),                 // 163: clientpb.Event
}
var file_rpcpb_services_proto_depIdxs = []int32{
	0,   // 0: rpcpb.SliverRPC.GetVersion:input_type -> commonpb.Empty
	0,   // 1: rpcpb.SliverRPC.GetOperators:input_type -> commonpb.Empty
	1,   // 2: rpcpb.SliverRPC.Kill:input_type -> sliverpb.KillReq
	2,   // 3: rpcpb.SliverRPC.Reconfigure:input_type -> sliverpb.ReconfigureReq
	3,   // 4: rpcpb.SliverRPC.Rename:input_type -> clientpb.RenameReq
	0,   // 5: rpcpb.SliverRPC.GetSessions:input_type -> commonpb.Empty
	0,   // 6: rpcpb.SliverRPC.GetBeacons:input_type -> commonpb.Empty
	4,   // 7: rpcpb.SliverRPC.GetBeacon:input_type -> clientpb.Beacon
	4,   // 8: rpcpb.SliverRPC.RmBeacon:input_type -> clientpb.Beacon
	4,   // 9: rpcpb.SliverRPC.GetBeaconTasks:input_type -> clientpb.Beacon
	5,   // 10: rpcpb.SliverRPC.GetBeaconTaskContent:input_type -> clientpb.BeaconTask
	0,   // 11: rpcpb.SliverRPC.MonitorStart:input_type -> commonpb.Empty
	0,   // 12: rpcpb.SliverRPC.MonitorStop:input_type -> commonpb.Empty
	0,   // 13: rpcpb.SliverRPC.GetJobs:input_type -> commonpb.Empty
	6,   // 14: rpcpb.SliverRPC.KillJob:input_type -> clientpb.KillJobReq
	7,   // 15: rpcpb.SliverRPC.StartMTLSListener:input_type -> clientpb.MTLSListenerReq
	8,   // 16: rpcpb.SliverRPC.StartWGListener:input_type -> clientpb.WGListenerReq
	9,   // 17: rpcpb.SliverRPC.StartDNSListener:input_type -> clientpb.DNSListenerReq
	10,  // 18: rpcpb.SliverRPC.StartHTTPSListener:input_type -> clientpb.HTTPListenerReq
	10,  // 19: rpcpb.SliverRPC.StartHTTPListener:input_type -> clientpb.HTTPListenerReq
	11,  // 20: rpcpb.SliverRPC.StartTCPStagerListener:input_type -> clientpb.StagerListenerReq
	11,  // 21: rpcpb.SliverRPC.StartHTTPStagerListener:input_type -> clientpb.StagerListenerReq
	12,  // 22: rpcpb.SliverRPC.LootAdd:input_type -> clientpb.Loot
	12,  // 23: rpcpb.SliverRPC.LootRm:input_type -> clientpb.Loot
	12,  // 24: rpcpb.SliverRPC.LootUpdate:input_type -> clientpb.Loot
	12,  // 25: rpcpb.SliverRPC.LootContent:input_type -> clientpb.Loot
	0,   // 26: rpcpb.SliverRPC.LootAll:input_type -> commonpb.Empty
	12,  // 27: rpcpb.SliverRPC.LootAllOf:input_type -> clientpb.Loot
	0,   // 28: rpcpb.SliverRPC.Hosts:input_type -> commonpb.Empty
	13,  // 29: rpcpb.SliverRPC.Host:input_type -> clientpb.Host
	13,  // 30: rpcpb.SliverRPC.HostRm:input_type -> clientpb.Host
	14,  // 31: rpcpb.SliverRPC.HostIOCRm:input_type -> clientpb.IOC
	15,  // 32: rpcpb.SliverRPC.Generate:input_type -> clientpb.GenerateReq
	16,  // 33: rpcpb.SliverRPC.Regenerate:input_type -> clientpb.RegenerateReq
	0,   // 34: rpcpb.SliverRPC.ImplantBuilds:input_type -> commonpb.Empty
	17,  // 35: rpcpb.SliverRPC.DeleteImplantBuild:input_type -> clientpb.DeleteReq
	0,   // 36: rpcpb.SliverRPC.Canaries:input_type -> commonpb.Empty
	0,   // 37: rpcpb.SliverRPC.GenerateWGClientConfig:input_type -> commonpb.Empty
	0,   // 38: rpcpb.SliverRPC.GenerateUniqueIP:input_type -> commonpb.Empty
	0,   // 39: rpcpb.SliverRPC.ImplantProfiles:input_type -> commonpb.Empty
	17,  // 40: rpcpb.SliverRPC.DeleteImplantProfile:input_type -> clientpb.DeleteReq
	18,  // 41: rpcpb.SliverRPC.SaveImplantProfile:input_type -> clientpb.ImplantProfile
	19,  // 42: rpcpb.SliverRPC.MsfStage:input_type -> clientpb.MsfStagerReq
	20,  // 43: rpcpb.SliverRPC.ShellcodeRDI:input_type -> clientpb.ShellcodeRDIReq
	0,   // 44: rpcpb.SliverRPC.GetCompiler:input_type -> commonpb.Empty
	0,   // 45: rpcpb.SliverRPC.Websites:input_type -> commonpb.Empty
	21,  // 46: rpcpb.SliverRPC.Website:input_type -> clientpb.Website
	21,  // 47: rpcpb.SliverRPC.WebsiteRemove:input_type -> clientpb.Website
	22,  // 48: rpcpb.SliverRPC.WebsiteAddContent:input_type -> clientpb.WebsiteAddContent
	22,  // 49: rpcpb.SliverRPC.WebsiteUpdateContent:input_type -> clientpb.WebsiteAddContent
	23,  // 50: rpcpb.SliverRPC.WebsiteRemoveContent:input_type -> clientpb.WebsiteRemoveContent
	24,  // 51: rpcpb.SliverRPC.Ping:input_type -> sliverpb.Ping
	25,  // 52: rpcpb.SliverRPC.Ps:input_type -> sliverpb.PsReq
	26,  // 53: rpcpb.SliverRPC.Terminate:input_type -> sliverpb.TerminateReq
	27,  // 54: rpcpb.SliverRPC.Ifconfig:input_type -> sliverpb.IfconfigReq
	28,  // 55: rpcpb.SliverRPC.Netstat:input_type -> sliverpb.NetstatReq
	29,  // 56: rpcpb.SliverRPC.Ls:input_type -> sliverpb.LsReq
	30,  // 57: rpcpb.SliverRPC.Cd:input_type -> sliverpb.CdReq
	31,  // 58: rpcpb.SliverRPC.Pwd:input_type -> sliverpb.PwdReq
	32,  // 59: rpcpb.SliverRPC.Rm:input_type -> sliverpb.RmReq
	33,  // 60: rpcpb.SliverRPC.Mkdir:input_type -> sliverpb.MkdirReq
	34,  // 61: rpcpb.SliverRPC.Download:input_type -> sliverpb.DownloadReq
	35,  // 62: rpcpb.SliverRPC.Upload:input_type -> sliverpb.UploadReq
	36,  // 63: rpcpb.SliverRPC.ProcessDump:input_type -> sliverpb.ProcessDumpReq
	37,  // 64: rpcpb.SliverRPC.RunAs:input_type -> sliverpb.RunAsReq
	38,  // 65: rpcpb.SliverRPC.Impersonate:input_type -> sliverpb.ImpersonateReq
	39,  // 66: rpcpb.SliverRPC.RevToSelf:input_type -> sliverpb.RevToSelfReq
	40,  // 67: rpcpb.SliverRPC.GetSystem:input_type -> clientpb.GetSystemReq
	41,  // 68: rpcpb.SliverRPC.Task:input_type -> sliverpb.TaskReq
	42,  // 69: rpcpb.SliverRPC.Msf:input_type -> clientpb.MSFReq
	43,  // 70: rpcpb.SliverRPC.MsfRemote:input_type -> clientpb.MSFRemoteReq
	44,  // 71: rpcpb.SliverRPC.ExecuteAssembly:input_type -> sliverpb.ExecuteAssemblyReq
	45,  // 72: rpcpb.SliverRPC.Migrate:input_type -> clientpb.MigrateReq
	46,  // 73: rpcpb.SliverRPC.Execute:input_type -> sliverpb.ExecuteReq
	47,  // 74: rpcpb.SliverRPC.ExecuteToken:input_type -> sliverpb.ExecuteTokenReq
	48,  // 75: rpcpb.SliverRPC.Sideload:input_type -> sliverpb.SideloadReq
	49,  // 76: rpcpb.SliverRPC.SpawnDll:input_type -> sliverpb.InvokeSpawnDllReq
	50,  // 77: rpcpb.SliverRPC.Screenshot:input_type -> sliverpb.ScreenshotReq
	51,  // 78: rpcpb.SliverRPC.PivotStartListener:input_type -> sliverpb.PivotStartListenerReq
	52,  // 79: rpcpb.SliverRPC.PivotStopListener:input_type -> sliverpb.PivotStopListenerReq
	53,  // 80: rpcpb.SliverRPC.PivotSessionListeners:input_type -> sliverpb.PivotListenersReq
	0,   // 81: rpcpb.SliverRPC.PivotGraph:input_type -> commonpb.Empty
	54,  // 82: rpcpb.SliverRPC.StartService:input_type -> sliverpb.StartServiceReq
	55,  // 83: rpcpb.SliverRPC.StopService:input_type -> sliverpb.StopServiceReq
	56,  // 84: rpcpb.SliverRPC.RemoveService:input_type -> sliverpb.RemoveServiceReq
	57,  // 85: rpcpb.SliverRPC.MakeToken:input_type -> sliverpb.MakeTokenReq
	58,  // 86: rpcpb.SliverRPC.GetEnv:input_type -> sliverpb.EnvReq
	59,  // 87: rpcpb.SliverRPC.SetEnv:input_type -> sliverpb.SetEnvReq
	60,  // 88: rpcpb.SliverRPC.UnsetEnv:input_type -> sliverpb.UnsetEnvReq
	61,  // 89: rpcpb.SliverRPC.Backdoor:input_type -> sliverpb.BackdoorReq
	62,  // 90: rpcpb.SliverRPC.RegistryRead:input_type -> sliverpb.RegistryReadReq
	63,  // 91: rpcpb.SliverRPC.RegistryWrite:input_type -> sliverpb.RegistryWriteReq
	64,  // 92: rpcpb.SliverRPC.RegistryCreateKey:input_type -> sliverpb.RegistryCreateKeyReq
	65,  // 93: rpcpb.SliverRPC.RegistryDeleteKey:input_type -> sliverpb.RegistryDeleteKeyReq
	66,  // 94: rpcpb.SliverRPC.RegistryListSubKeys:input_type -> sliverpb.RegistrySubKeyListReq
	67,  // 95: rpcpb.SliverRPC.RegistryListValues:input_type -> sliverpb.RegistryListValuesReq
	68,  // 96: rpcpb.SliverRPC.RunSSHCommand:input_type -> sliverpb.SSHCommandReq
	69,  // 97: rpcpb.SliverRPC.HijackDLL:input_type -> clientpb.DllHijackReq
	70,  // 98: rpcpb.SliverRPC.GetPrivs:input_type -> sliverpb.GetPrivsReq
	71,  // 99: rpcpb.SliverRPC.OpenSession:input_type -> sliverpb.OpenSession
	72,  // 100: rpcpb.SliverRPC.CloseSession:input_type -> sliverpb.CloseSession
	73,  // 101: rpcpb.SliverRPC.RegisterExtension:input_type -> sliverpb.RegisterExtensionReq
	74,  // 102: rpcpb.SliverRPC.CallExtension:input_type -> sliverpb.CallExtensionReq
	75,  // 103: rpcpb.SliverRPC.ListExtensions:input_type -> sliverpb.ListExtensionsReq
	76,  // 104: rpcpb.SliverRPC.WGStartPortForward:input_type -> sliverpb.WGPortForwardStartReq
	77,  // 105: rpcpb.SliverRPC.WGStopPortForward:input_type -> sliverpb.WGPortForwardStopReq
	78,  // 106: rpcpb.SliverRPC.WGStartSocks:input_type -> sliverpb.WGSocksStartReq
	79,  // 107: rpcpb.SliverRPC.WGStopSocks:input_type -> sliverpb.WGSocksStopReq
	80,  // 108: rpcpb.SliverRPC.WGListForwarders:input_type -> sliverpb.WGTCPForwardersReq
	81,  // 109: rpcpb.SliverRPC.WGListSocksServers:input_type -> sliverpb.WGSocksServersReq
	82,  // 110: rpcpb.SliverRPC.Shell:input_type -> sliverpb.ShellReq
	83,  // 111: rpcpb.SliverRPC.Portfwd:input_type -> sliverpb.PortfwdReq
	84,  // 112: rpcpb.SliverRPC.CreateSocks:input_type -> sliverpb.Socks
	84,  // 113: rpcpb.SliverRPC.CloseSocks:input_type -> sliverpb.Socks
	85,  // 114: rpcpb.SliverRPC.SocksProxy:input_type -> sliverpb.SocksData
	86,  // 115: rpcpb.SliverRPC.CreateTunnel:input_type -> sliverpb.Tunnel
	86,  // 116: rpcpb.SliverRPC.CloseTunnel:input_type -> sliverpb.Tunnel
	87,  // 117: rpcpb.SliverRPC.TunnelData:input_type -> sliverpb.TunnelData
	0,   // 118: rpcpb.SliverRPC.Events:input_type -> commonpb.Empty
	88,  // 119: rpcpb.SliverRPC.GetVersion:output_type -> clientpb.Version
	89,  // 120: rpcpb.SliverRPC.GetOperators:output_type -> clientpb.Operators
	0,   // 121: rpcpb.SliverRPC.Kill:output_type -> commonpb.Empty
	90,  // 122: rpcpb.SliverRPC.Reconfigure:output_type -> sliverpb.Reconfigure
	0,   // 123: rpcpb.SliverRPC.Rename:output_type -> commonpb.Empty
	91,  // 124: rpcpb.SliverRPC.GetSessions:output_type -> clientpb.Sessions
	92,  // 125: rpcpb.SliverRPC.GetBeacons:output_type -> clientpb.Beacons
	4,   // 126: rpcpb.SliverRPC.GetBeacon:output_type -> clientpb.Beacon
	0,   // 127: rpcpb.SliverRPC.RmBeacon:output_type -> commonpb.Empty
	93,  // 128: rpcpb.SliverRPC.GetBeaconTasks:output_type -> clientpb.BeaconTasks
	5,   // 129: rpcpb.SliverRPC.GetBeaconTaskContent:output_type -> clientpb.BeaconTask
	94,  // 130: rpcpb.SliverRPC.MonitorStart:output_type -> commonpb.Response
	0,   // 131: rpcpb.SliverRPC.MonitorStop:output_type -> commonpb.Empty
	95,  // 132: rpcpb.SliverRPC.GetJobs:output_type -> clientpb.Jobs
	96,  // 133: rpcpb.SliverRPC.KillJob:output_type -> clientpb.KillJob
	97,  // 134: rpcpb.SliverRPC.StartMTLSListener:output_type -> clientpb.MTLSListener
	98,  // 135: rpcpb.SliverRPC.StartWGListener:output_type -> clientpb.WGListener
	99,  // 136: rpcpb.SliverRPC.StartDNSListener:output_type -> clientpb.DNSListener
	100, // 137: rpcpb.SliverRPC.StartHTTPSListener:output_type -> clientpb.HTTPListener
	100, // 138: rpcpb.SliverRPC.StartHTTPListener:output_type -> clientpb.HTTPListener
	101, // 139: rpcpb.SliverRPC.StartTCPStagerListener:output_type -> clientpb.StagerListener
	101, // 140: rpcpb.SliverRPC.StartHTTPStagerListener:output_type -> clientpb.StagerListener
	12,  // 141: rpcpb.SliverRPC.LootAdd:output_type -> clientpb.Loot
	0,   // 142: rpcpb.SliverRPC.LootRm:output_type -> commonpb.Empty
	12,  // 143: rpcpb.SliverRPC.LootUpdate:output_type -> clientpb.Loot
	12,  // 144: rpcpb.SliverRPC.LootContent:output_type -> clientpb.Loot
	102, // 145: rpcpb.SliverRPC.LootAll:output_type -> clientpb.AllLoot
	102, // 146: rpcpb.SliverRPC.LootAllOf:output_type -> clientpb.AllLoot
	103, // 147: rpcpb.SliverRPC.Hosts:output_type -> clientpb.AllHosts
	13,  // 148: rpcpb.SliverRPC.Host:output_type -> clientpb.Host
	0,   // 149: rpcpb.SliverRPC.HostRm:output_type -> commonpb.Empty
	0,   // 150: rpcpb.SliverRPC.HostIOCRm:output_type -> commonpb.Empty
	104, // 151: rpcpb.SliverRPC.Generate:output_type -> clientpb.Generate
	104, // 152: rpcpb.SliverRPC.Regenerate:output_type -> clientpb.Generate
	105, // 153: rpcpb.SliverRPC.ImplantBuilds:output_type -> clientpb.ImplantBuilds
	0,   // 154: rpcpb.SliverRPC.DeleteImplantBuild:output_type -> commonpb.Empty
	106, // 155: rpcpb.SliverRPC.Canaries:output_type -> clientpb.Canaries
	107, // 156: rpcpb.SliverRPC.GenerateWGClientConfig:output_type -> clientpb.WGClientConfig
	108, // 157: rpcpb.SliverRPC.GenerateUniqueIP:output_type -> clientpb.UniqueWGIP
	109, // 158: rpcpb.SliverRPC.ImplantProfiles:output_type -> clientpb.ImplantProfiles
	0,   // 159: rpcpb.SliverRPC.DeleteImplantProfile:output_type -> commonpb.Empty
	18,  // 160: rpcpb.SliverRPC.SaveImplantProfile:output_type -> clientpb.ImplantProfile
	110, // 161: rpcpb.SliverRPC.MsfStage:output_type -> clientpb.MsfStager
	111, // 162: rpcpb.SliverRPC.ShellcodeRDI:output_type -> clientpb.ShellcodeRDI
	112, // 163: rpcpb.SliverRPC.GetCompiler:output_type -> clientpb.Compiler
	113, // 164: rpcpb.SliverRPC.Websites:output_type -> clientpb.Websites
	21,  // 165: rpcpb.SliverRPC.Website:output_type -> clientpb.Website
	0,   // 166: rpcpb.SliverRPC.WebsiteRemove:output_type -> commonpb.Empty
	21,  // 167: rpcpb.SliverRPC.WebsiteAddContent:output_type -> clientpb.Website
	21,  // 168: rpcpb.SliverRPC.WebsiteUpdateContent:output_type -> clientpb.Website
	21,  // 169: rpcpb.SliverRPC.WebsiteRemoveContent:output_type -> clientpb.Website
	24,  // 170: rpcpb.SliverRPC.Ping:output_type -> sliverpb.Ping
	114, // 171: rpcpb.SliverRPC.Ps:output_type -> sliverpb.Ps
	115, // 172: rpcpb.SliverRPC.Terminate:output_type -> sliverpb.Terminate
	116, // 173: rpcpb.SliverRPC.Ifconfig:output_type -> sliverpb.Ifconfig
	117, // 174: rpcpb.SliverRPC.Netstat:output_type -> sliverpb.Netstat
	118, // 175: rpcpb.SliverRPC.Ls:output_type -> sliverpb.Ls
	119, // 176: rpcpb.SliverRPC.Cd:output_type -> sliverpb.Pwd
	119, // 177: rpcpb.SliverRPC.Pwd:output_type -> sliverpb.Pwd
	120, // 178: rpcpb.SliverRPC.Rm:output_type -> sliverpb.Rm
	121, // 179: rpcpb.SliverRPC.Mkdir:output_type -> sliverpb.Mkdir
	122, // 180: rpcpb.SliverRPC.Download:output_type -> sliverpb.Download
	123, // 181: rpcpb.SliverRPC.Upload:output_type -> sliverpb.Upload
	124, // 182: rpcpb.SliverRPC.ProcessDump:output_type -> sliverpb.ProcessDump
	125, // 183: rpcpb.SliverRPC.RunAs:output_type -> sliverpb.RunAs
	126, // 184: rpcpb.SliverRPC.Impersonate:output_type -> sliverpb.Impersonate
	127, // 185: rpcpb.SliverRPC.RevToSelf:output_type -> sliverpb.RevToSelf
	128, // 186: rpcpb.SliverRPC.GetSystem:output_type -> sliverpb.GetSystem
	129, // 187: rpcpb.SliverRPC.Task:output_type -> sliverpb.Task
	129, // 188: rpcpb.SliverRPC.Msf:output_type -> sliverpb.Task
	129, // 189: rpcpb.SliverRPC.MsfRemote:output_type -> sliverpb.Task
	130, // 190: rpcpb.SliverRPC.ExecuteAssembly:output_type -> sliverpb.ExecuteAssembly
	131, // 191: rpcpb.SliverRPC.Migrate:output_type -> sliverpb.Migrate
	132, // 192: rpcpb.SliverRPC.Execute:output_type -> sliverpb.Execute
	132, // 193: rpcpb.SliverRPC.ExecuteToken:output_type -> sliverpb.Execute
	133, // 194: rpcpb.SliverRPC.Sideload:output_type -> sliverpb.Sideload
	134, // 195: rpcpb.SliverRPC.SpawnDll:output_type -> sliverpb.SpawnDll
	135, // 196: rpcpb.SliverRPC.Screenshot:output_type -> sliverpb.Screenshot
	136, // 197: rpcpb.SliverRPC.PivotStartListener:output_type -> sliverpb.PivotListener
	0,   // 198: rpcpb.SliverRPC.PivotStopListener:output_type -> commonpb.Empty
	137, // 199: rpcpb.SliverRPC.PivotSessionListeners:output_type -> sliverpb.PivotListeners
	138, // 200: rpcpb.SliverRPC.PivotGraph:output_type -> clientpb.PivotGraph
	139, // 201: rpcpb.SliverRPC.StartService:output_type -> sliverpb.ServiceInfo
	139, // 202: rpcpb.SliverRPC.StopService:output_type -> sliverpb.ServiceInfo
	139, // 203: rpcpb.SliverRPC.RemoveService:output_type -> sliverpb.ServiceInfo
	140, // 204: rpcpb.SliverRPC.MakeToken:output_type -> sliverpb.MakeToken
	141, // 205: rpcpb.SliverRPC.GetEnv:output_type -> sliverpb.EnvInfo
	142, // 206: rpcpb.SliverRPC.SetEnv:output_type -> sliverpb.SetEnv
	143, // 207: rpcpb.SliverRPC.UnsetEnv:output_type -> sliverpb.UnsetEnv
	144, // 208: rpcpb.SliverRPC.Backdoor:output_type -> sliverpb.Backdoor
	145, // 209: rpcpb.SliverRPC.RegistryRead:output_type -> sliverpb.RegistryRead
	146, // 210: rpcpb.SliverRPC.RegistryWrite:output_type -> sliverpb.RegistryWrite
	147, // 211: rpcpb.SliverRPC.RegistryCreateKey:output_type -> sliverpb.RegistryCreateKey
	148, // 212: rpcpb.SliverRPC.RegistryDeleteKey:output_type -> sliverpb.RegistryDeleteKey
	149, // 213: rpcpb.SliverRPC.RegistryListSubKeys:output_type -> sliverpb.RegistrySubKeyList
	150, // 214: rpcpb.SliverRPC.RegistryListValues:output_type -> sliverpb.RegistryValuesList
	151, // 215: rpcpb.SliverRPC.RunSSHCommand:output_type -> sliverpb.SSHCommand
	152, // 216: rpcpb.SliverRPC.HijackDLL:output_type -> clientpb.DllHijack
	153, // 217: rpcpb.SliverRPC.GetPrivs:output_type -> sliverpb.GetPrivs
	71,  // 218: rpcpb.SliverRPC.OpenSession:output_type -> sliverpb.OpenSession
	0,   // 219: rpcpb.SliverRPC.CloseSession:output_type -> commonpb.Empty
	154, // 220: rpcpb.SliverRPC.RegisterExtension:output_type -> sliverpb.RegisterExtension
	155, // 221: rpcpb.SliverRPC.CallExtension:output_type -> sliverpb.CallExtension
	156, // 222: rpcpb.SliverRPC.ListExtensions:output_type -> sliverpb.ListExtensions
	157, // 223: rpcpb.SliverRPC.WGStartPortForward:output_type -> sliverpb.WGPortForward
	157, // 224: rpcpb.SliverRPC.WGStopPortForward:output_type -> sliverpb.WGPortForward
	158, // 225: rpcpb.SliverRPC.WGStartSocks:output_type -> sliverpb.WGSocks
	158, // 226: rpcpb.SliverRPC.WGStopSocks:output_type -> sliverpb.WGSocks
	159, // 227: rpcpb.SliverRPC.WGListForwarders:output_type -> sliverpb.WGTCPForwarders
	160, // 228: rpcpb.SliverRPC.WGListSocksServers:output_type -> sliverpb.WGSocksServers
	161, // 229: rpcpb.SliverRPC.Shell:output_type -> sliverpb.Shell
	162, // 230: rpcpb.SliverRPC.Portfwd:output_type -> sliverpb.Portfwd
	84,  // 231: rpcpb.SliverRPC.CreateSocks:output_type -> sliverpb.Socks
	0,   // 232: rpcpb.SliverRPC.CloseSocks:output_type -> commonpb.Empty
	85,  // 233: rpcpb.SliverRPC.SocksProxy:output_type -> sliverpb.SocksData
	86,  // 234: rpcpb.SliverRPC.CreateTunnel:output_type -> sliverpb.Tunnel
	0,   // 235: rpcpb.SliverRPC.CloseTunnel:output_type -> commonpb.Empty
	87,  // 236: rpcpb.SliverRPC.TunnelData:output_type -> sliverpb.TunnelData
	163, // 237: rpcpb.SliverRPC.Events:output_type -> clientpb.Event
	119, // [119:238] is the sub-list for method output_type
	0,   // [0:119] is the sub-list for method input_type
	0,   // [0:0] is the sub-list for extension type_name
	0,   // [0:0] is the sub-list for extension extendee
	0,   // [0:0] is the sub-list for field type_name
}

func init() { file_rpcpb_services_proto_init() }
func file_rpcpb_services_proto_init() {
	if File_rpcpb_services_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_rpcpb_services_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_rpcpb_services_proto_goTypes,
		DependencyIndexes: file_rpcpb_services_proto_depIdxs,
	}.Build()
	File_rpcpb_services_proto = out.File
	file_rpcpb_services_proto_rawDesc = nil
	file_rpcpb_services_proto_goTypes = nil
	file_rpcpb_services_proto_depIdxs = nil
}
