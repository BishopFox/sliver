...
func (n *regexNode) hasOptions(mask RegexOptions) bool {
	if n.options&mask != 0 {
		return true
	}
	for _, child := range n.children {
		if child.hasOptions(mask) {
			return true
		}
	}
	return false
	...
	for i, j = 0, 0; i < len(n.children); i, j = i+1, j+1 {
		...
		for {
			if at.t == ntAlternate {
				...
				optionsAt := at.options & (RightToLeft | IgnoreCase | JavaASCII | JavaUnicode)
...
func (n *regexNode) reduceSet() *regexNode {
	...
	if n.set == nil {
		n.t = ntNothing
	} else if n.options&customCaseOptions != 0 {
		// Java case modes need the original set provenance.  A singleton such
		// as [dotless-i] has more inverse equivalents than its folded key alone.
		return n
...
func (n *regexNode) description() string {
	...
	if (n.options & JavaASCII) != 0 {
		buf.WriteString("-A")
	}
	if (n.options & JavaUnicode) != 0 {
		buf.WriteString("-J")
	}
