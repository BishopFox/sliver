...
If there are any advanced options you need to pass to msfvenom, you can use the --advanced flag to provide them. They must be provided in URI query format:
option1=value1&option2=value2 and so on. The full list of advanced options is available using "show advanced" in msf for the payload corresponding to the
chosen protocol:
...
Example:
	To tell the stager to use the proxy proxy.corp.com:8080 with the user name "corp_drone" and password "MyPassword", you would pass the following string
	to --advanced: HttpProxyHost=proxy.corp.com&HttpProxyPort=8080&HttpProxyUser=corp_drone&HttpProxyPass=MyPassword
`
