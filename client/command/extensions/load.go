...
func ExtensionLoadCmd(ctx *grumble.Context, con *console.SliverConsoleClient) {
	...
	extCmd, err := extensions.LoadExtensionManifest(filepath.Join(dirPath, extensions.ManifestFileName))
	if err != nil {
		return
	}
...
func ExtensionRegisterCommand(extCmd *extensions.ExtensionManifest, con *console.SliverConsoleClient) {
	extensions.AddExtension(extCmd)
	helpMsg := extCmd.Help
...
func runExtensionCmd(ctx *grumble.Context, con *console.SliverConsoleClient) {
	...
	if isBOF {
		...
		entryPoint = depExt.Entrypoint // should exist at this point
	} else {
		// Regular DLL
...
func getBOFArgs(ctx *grumble.Context, binPath string, ext *extensions.ExtensionManifest) ([]byte, error) {
