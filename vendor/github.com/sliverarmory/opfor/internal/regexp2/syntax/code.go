...
const (
	...
	Ci       = 512  // bit to indicate regexp2's stock case-insensitive mode.
	CiASCII  = 1024 // bit to indicate Java ASCII-only case-insensitive mode.
	CiJava   = 2048 // bit to indicate Java Unicode case-insensitive mode.
	CaseMask = Ci | CiASCII | CiJava
)
...
func opcodeSize(op InstOp) int {
	...
	switch op {
	case Nothing, Bol, Eol, Boundary, Nonboundary, ECMABoundary, NonECMABoundary, Grapheme,
		GraphemeBoundary, Beginning, Start, EndZ, End, Nullmark, Setmark, Getmark, Setjump, Backjump,
		Forejump, Stop:
		return 1
