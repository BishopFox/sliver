...
func renderGlobalMarkdown(report GlobalReport) []byte {
	var output strings.Builder
	output.WriteString("# Sliver rportfwd E2E coverage\n\n")
...
func renderMatrixTable(output *strings.Builder, rows []MatrixRow, includeTarget bool) {
	...
	for _, scenario := range fixedScenarios {
		fmt.Fprintf(output, " %s |", markdown(scenario))
	}
	output.WriteByte('\n')
	...
	for _, row := range rows {
		...
		fmt.Fprintf(output, " %s |", markdown(row.Transport))
...
func renderLegend(output *strings.Builder) {
	output.WriteString("Legend: ✅ PASS · ❌ FAIL · ⚪ NOT RUN (required and missing)\n\n")
}

func renderSummary(output *strings.Builder, summary Summary) {
	output.WriteString("## Summary\n\n")
	output.WriteString("| Required | Recorded | Pass | Fail | Not run | Total cells |\n")
	output.WriteString("|---:|---:|---:|---:|---:|---:|\n")
	fmt.Fprintf(
		output,
		"| %d | %d | %d | %d | %d | %d |\n\n",
...
func renderFailureDetails(output *strings.Builder, failed []Record, missing []Identity) {
	...
	if len(failed) > 0 {
		...
		output.WriteString("| Target | Transport | Scenario | Duration | Detail |\n")
		output.WriteString("|---|---|---|---:|---|\n")
	...
	if len(missing) > 0 {
		...
		output.WriteString("| Target | Transport | Scenario | Status |\n")
		output.WriteString("|---|---|---|---:|\n")
