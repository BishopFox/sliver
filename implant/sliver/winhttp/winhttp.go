...
func Open(pszAgentW Lpwstr, dwAccessType Dword, pszProxyW Lpwstr, pszProxyBypassW Lpwstr, dwFlags Dword) (HInternet, error) {
	openP, err := whd.FindProc("WinHttpOpen")
	if err != nil {
		return 0, err
	}
	r, _, err := openP.Call(
...
func CloseHandle(hInternet HInternet) error {
	closeHandleP, err := whd.FindProc("WinHttpCloseHandle")
	if err != nil {
		return err
	}
...
func SetTimeouts(hInternet HInternet, nResolveTimeout int, nConnectTimeout int, nSendTimeout int, nReceiveTimeout int) error {
	setTimeoutsP, err := whd.FindProc("WinHttpSetTimeouts")
	if err != nil {
		return err
	}
...
func GetProxyForUrl(hInternet HInternet, lpcwszUrl Lpwstr, pAutoProxyOptions *AutoProxyOptions) (*ProxyInfo, error) {
	getProxyForUrlP, err := whd.FindProc("WinHttpGetProxyForUrl")
	if err != nil {
		return nil, err
	}
	p := new(ProxyInfo)
...
func GetIEProxyConfigForCurrentUser() (*CurrentUserIEProxyConfig, error) {
	getIEProxyConfigForCurrentUserP, err := whd.FindProc("WinHttpGetIEProxyConfigForCurrentUser")
	if err != nil {
		return nil, err
	}
...
func GetDefaultProxyConfiguration() (*ProxyInfo, error) {
	pInfo := new(ProxyInfo)
	getDefaultProxyConfigurationP, err := whd.FindProc("WinHttpGetDefaultProxyConfiguration")
	if err != nil {
		return nil, err
	}
...
func globalFree(hMem *uint16) error {
	...
	globalFreeP, err := kd.FindProc("GlobalFree")
	if err != nil {
		return err
	}
